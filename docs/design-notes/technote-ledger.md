# Technote Design Ledger

Moved on 2026-09-20 from chapter 9 of the public technote (`web/content/technote.md`).
This file keeps the design decisions, upstream revisions, numerical registrations and
...
example niche separation through crowding between Voices or macro-scale form, has been left
to ALife emergence and to the Scenario. The author adopted the same division as an explicit
rule for temporal structure. The 2026-09-13 removal of I9 is now read as a single
application of this rule.

**Selection rule.** A temporal function is placed on the DCC side (shared observation,
expectation and memory, and the path that returns them to generation) only if all three
conditions hold:

1. The target listening response has primary-research evidence and an identified
   neurocognitive mechanism.
2. The computational model is either identified in existing literature or can be stated
...
default-off passive diagnostics (report/UI) may be kept as exceptions. The effect on sound
is left to local ALife decisions and authored Scenario structure; ListenerTwin keeps
passive observation indicators only.

**Result of applying the rule.** Each condition was judged separately per function.
"Not identified" refers to the candidates examined for this plan, not to future research.

| Function | Decision | Condition 1 (response, mechanism) | Condition 2 (model) | Condition 3 (decision) |
|---|---|---|---|---|
| Articulation: observation of heard gestures (I5, shared P) | Core | Auditory–premotor sensorimotor correspondence | Explicit, uncalibrated attack/continuation/release/gap scales | Author audition |
| Articulation: action-conditioned self-sound prediction (I10, Voice-private A) | Core | Schneider 2018, Audette 2022 and others | Finite-parameter error update; model, scales and window registered before implementation | Author audition |
| Beat, arrival, measure (I6) | Core | Beat: putamen/SMA/premotor beat network (Grahn & Rowe 2009). Measure/metrical interpretation: imagined meter changes EEG responses (Nozaradan 2011), kept as an interpretation state distinct from the pulse | Period and arrival forecasts on the observed-accent clock, explicit thresholds | Author audition |
| Short-term recurrence expectation and habituation (existing recurrence/band-energy prediction, recall retention state and bounded causal references of reference_inventory) | Core | Behavioral evidence for memory decay (Harrison 2020) and implicit memory (Bianco 2020); candidate mechanism is stimulus-specific adaptation in primary auditory cortex (Ulanovsky 2003), correspondence partial; core adoption limited to the existing implementation | Explicit rules of the existing implementation | Author audition |
| Groove / participation-desire rating head (T3; 54 inputs, 109-coefficient ordinal logit) | Research extension | Behavioral evidence (Witek 2014 and others); mechanism partial | Coefficients presuppose fitting to human ratings; not an existing model | R4/R5 need human ratings |
| Correspondence through transformation, episodic retrieval (I4 matcher, memory search, recall graph) | Research extension | Implicit-memory evidence exists | DTW and the episode graph are new representations of this plan | Fitting needs human ratings |
| Phrase and closure (T5, I7) | Research extension | Boundary responses have evidence; a closure mechanism is not identified among the examined candidates | Presupposes an 82-component fit | Human ratings needed |
| Section and return (T6, I8) | Research extension | Event-boundary and context-reactivation evidence, not music-specific | Memory-attention graph is a design hypothesis; 39-component fit | Human ratings needed |
| Whole-piece context (T7, I9) | Research extension | 2026-09-13 decision stands | Same | Same |

Within T3, intrinsic pace, participation period and the sound/wait/skip decision already
exist on the ALife side as the Voice's body and the author's participation disposition;
the core uses them as they are. The perceptual groove rating head is kept as a default-off
diagnostic enabled only by an explicit `[temporal_groove]` section.

Accordingly, of the four interactions only I12b (beat/grouping ↔ gesture) stays in the core,
restricted to two paths: I6 arrival expectation acting on gesture candidate timing, and
observed accents feeding period estimation as they already do. Ablation comparisons run per
path. The update rule, thresholds and scales of the former path are registered before the I11
implementation; until then its core adoption is held and only the latter, existing path is
active. No independent grouping inference is added to the core. I12a, I12c and I12d join I12e in the research extension. Research gates R1 (human
data collection including institutional procedures), R4 (fitting and calibration) and R5
...
scope and gates only; Rust and generated audio are unchanged. The execution ledger is
`docs/roadmap/temporal-dcc/milestones.md` §1.3.
...
[The hierarchy specification §3.1](../design-notes/dcc-neurocognitive-hierarchy.md#31-adopted-circuit-and-embodied-affordance-interpretation-2026-09-12)
...
gates. The [implementation record](../roadmap/temporal-dcc/m0-rust-preparation.md)
...
[fit preparation record](../roadmap/temporal-dcc/m0-ordinal-fit-preparation.md)
...
The [rating preparation record](../roadmap/temporal-dcc/m0-rating-preparation.md)
...
effects remain absent. The [observation record](../roadmap/temporal-dcc/m1-observation.md)
...
[trajectory input record](../roadmap/temporal-dcc/m1-trajectory-input.md)
...
The [ridge preparation record](../roadmap/temporal-dcc/m0-ridge-preparation.md)
...
see the [assignment record](../roadmap/temporal-dcc/m0-group-assignment.md).
...
[grouping-window record](../roadmap/temporal-dcc/m0-grouping-window.md).
...
capacity. The [proposal record](../roadmap/temporal-dcc/m0-proposal-persistence.md)
...
[producer record](../roadmap/temporal-dcc/m0-proposal-generation.md).
...
[lifecycle record](../roadmap/temporal-dcc/m0-group-lifecycle.md).
...
[composed frontend record](../roadmap/temporal-dcc/m0-acoustic-frontend.md).
...
[raw feature record](../roadmap/temporal-dcc/m0-raw-features.md).
...
[accent ledger record](../roadmap/temporal-dcc/m0-accent-ledger.md).
...
[period grid record](../roadmap/temporal-dcc/m0-period-grid.md).
...
[grouping inventory record](../roadmap/temporal-dcc/m0-grouping-inventory.md).
...
[recurrence ownership record](../roadmap/temporal-dcc/m0-recurrence-ownership.md).
