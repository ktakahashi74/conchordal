Now I have all the information needed. Based on my review of the codebase, I can see that:

1. The threading model in `src/app.rs` shows 4 thread contexts: Audio Thread, Harmonicity Worker, Roughness Worker, and Worker/GUI Thread
2. The sample paths in `samples/` directory match the structure in the technote
3. The worker files are in `src/core/stream/*.rs` (dorsal.rs, harmonicity.rs, roughness.rs)
4. Audio output is in `src/audio/output.rs`

The technote.md is largely accurate. I need to make minimal changes to ensure the threading model description matches the actual code. Let me output the updated technote.md:
...
source_commit = ""
source_snapshot = "2025-12-26T00:00:00Z"
...
1.  **Audio Thread (Real-Time Priority)**:
    ...
    *   **Responsibility**: Pulls samples from the ring buffer and pushes to the hardware. The actual sample generation happens in the Worker thread.

2.  **Harmonicity Worker (Background Priority)**:
    *   Defined in `core/stream/harmonicity.rs` and driven by `core/harmonicity_worker.rs`.
...
3.  **Roughness Worker (Background Priority)**:
    *   Defined in `core/stream/roughness.rs` and driven by `core/roughness_worker.rs`.
    *   **Responsibility**: Receives audio chunks and computes the Roughness field via ERB-domain convolution using the NSGT-RT pipeline.
    *   **Update Cycle**: Sends updated Roughness data back to the main loop via a separate SPSC channel.

4.  **Worker Thread (Main Processing)**:
    *   Spawned in `app.rs` as the "worker" thread.
    *   **Responsibility**: Runs the main simulation loop. It handles the `Conductor` (script execution), `Population` (agent lifecycle), audio sample generation, and `DorsalStream` (rhythm analysis). It coordinates with the Harmonicity and Roughness workers via channels, merges their results into the `Landscape`, and sends UI frames to the GUI.
    *   **DorsalStream**: Rhythm analysis (`core/stream/dorsal.rs`) runs synchronously within this thread, processing audio chunks to extract rhythmic energy metrics (e_low, e_mid, e_high, flux) for the `NeuralRhythms` modulation bank.
...
3.  The **Worker Thread** receives updates from both workers via separate SPSC channels and merges them into the `LandscapeFrame`.
4.  The `Population` holds the "current" Landscape. When new data arrives from either worker, the worker thread updates the corresponding field and recomputes the combined Consonance.
