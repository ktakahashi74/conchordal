...
sample 12では24秒に探索冷却、glide切替、基準音の帰還、flowの減音が重なる。
作者の補足を受け、次の試聴はglide ONと基準音の帰還を共通にし、flowの減音の有無だけを比較する2条件に絞る。
以下は次の実験計画であり、まだ音源を生成していない。
...
介入前[0,24)秒のPCMと状態記録の一致を検査する。
まず1seedの2条件で、同じ前半を聴いてから24〜27.3秒の変化を比べる。再生音量は固定し、音源別の音量正規化は行わない。
27.3秒のflow解放以降は別の操作を含むため、この比較窓には含めない。
条件名を隠したA/B表示を用い、まず差が聞き取れるか、次に協和への落ち着きの印象が変わるかを
「判断できない」も含めて記録する。提示順と条件の対応も保存する。
この比較は24秒のflowの減音を対象とし、glide単独の効果や28.6秒のcolonyの減音には一般化しない。
...
今回の判断保留だけを根拠に、新しい期待・終止モデルが必要とは判断しない。
