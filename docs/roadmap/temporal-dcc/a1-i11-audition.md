...
## 0. 2026-09-24の再登録範囲と停止点

設計台帳の[二層化と時間地形](../../design-notes/technote-ledger.ja.md#two-tier-rule)に従い、**現行版の60本は本体A1の受入対象から外す**。§10の実パス・SHAと旧登録の判断履歴は削除しない。作者が選んだ旧60本・4 session各15試行の実施を、受入対象から外す判断だけで自動的に「中止確定」とはしない。非受入の手順確認・基準試聴として残すか、取りやめるかは作者未決である。推奨は、素材を保管し、提示器の必要が生じた時だけ非受入の手順確認に限定して使い、旧60本の本番試聴は行わない案である。理由は、旧版への4 sessionの聴取と条件記憶を増やしても地形を入れた版の受入証拠にはならず、新版の盲検を弱めるためである。本人の判断前に素材を消去せず、試聴も開始しない。

本体A1の順序は、T1のERB帯域同時・前方マスキング地形を入れた**T2地形前版**でT1を判定し、その版を固定した後にT2の包絡変調スペクトル地形を入れ、I11-2／I12bを新版で再登録・再取得してからT2を判定する。T1とT2のA1は対象版を明記して並行可能である。T2変更後にはT1の入力、状態介入、生成分岐、off基準、音声・関係記述を登録した回帰で照合し、同じ証拠が適用できない条件のT1 A1は統合版で再実施する。A1の対象版はR2の統合版と同一だと推定しない。T1／T2の担当IをI11-3／I11-4とする案は作者未承認である。

### 0.1 T1新版の素材と比較の草案

素材単位は、旧I11-1の登録12条件（sine-hold／sine-flow／harmonic-flow／modal-flow × 4／16／64 Voice）を、**T1地形前の封印版**と**T1地形後・T2地形前の版**で対応させる案である。各条件につき、旧版の`body` 1本、新版の`body`・`proxy`・`masking-connection-cut`各1本を同じ登録入力、seed、代表record、route、長さで取得し、計48本を予定する。旧版の`body`対新版の`body`の12対は版変更の記述的比較だけに使う。新版内の`body`対`proxy`12対をfootprint介入、新版内の`body`対`masking-connection-cut`12対を接続除去とし、旧新版比較で因果介入・接続除去を代用しない。接続除去腕はマスキング地形の計算と入力受領を維持し、参加費用への重なり項だけを無効化する案である。旧`none`のような複数経路の一括除去を厳密な接続除去と呼ばない。切断境界と状態保持、最初の分岐は実装前登録と検査器で固定する。

主作用層はharmonic／modal flow × 4／16／64の6条件とし、各比較の主分母は**6対**。sine-flow 3条件とsine-hold 3条件は、比較ごとに主作用層から分けて報告する。sine-flowの旧版での無差証拠は`body`対`proxy`に限られ、新版にもそのまま継承しない。新版ではモデル選定後に、両腕へ渡すfootprintとマスキング入力の同一性を確認できた場合だけ、footprint介入の無差対照として登録する。確認できなければ比較条件として残し、無差を成功条件にしない。`body`対`masking-connection-cut`では、sine-flowも費用項の除去で変わり得るため、無差対照とはしない。sine-holdは参加決定を使わない負対照の候補とし、切断境界がholdへ作用しないことを取得前に確認する。旧新版比較のsine条件には無差を要求しない。主6対をWAV差や聴取結果で選別しない。各対の素材欠損・SHA不一致・無分岐・不明回答は理由と共に残し、主分母を縮めない。登録した負対照の不一致を主作用の成功で相殺しない。旧版の`proxy`や`none`、§10の60本を新版の条件へ混入しない。

この48本は**取得前の行列案**であり、既存素材が揃ったという意味ではない。旧版の比較基準はI12bの終結後に固定するsource／binary／設定とし、T1モデル・パラメータ、版commit、binary SHA、入力・seed・代表record・WAV path／SHA、writer条件、接続除去の実装、提示器の版は未記入である。比較の完全性には入力一致、状態介入、期限、off基準、`None`のbit一致と、モデルの二層分類を要求する。取得前には、生成条件、比較腕、対照の期待、検査規則を確定し、独立Astraレビュー「可」の後に取得仕様を凍結する。その後に音声を取得・検査し、実素材のpathと全SHAを確定する。素材台帳と提示手順を含む提示前登録も独立Astraレビュー「可」の後に凍結し、作者の試聴へ渡す。今回の草案保存はどちらの凍結も意味しない。

### 0.2 T1新版の提示と判定の草案

作者一人の記述的検査とする。48本を4 session各12試行に分け、各sessionに12条件を1本ずつ、四つの腕（旧body、新body、新proxy、新cut）を各3本配置する。各条件の四腕は別sessionに置き、提示順は秘匿seedで制約内乱数化する。主作用層6条件では四腕の各session配分を1本または2本とし、新bodyが先になる主介入対と後になる対を3対ずつにする。session間隔48時間以上、全長再生は各試行最大2回、途中の答え合わせなしを第一案とする。旧新版比較、新版内介入、新版内除去の各対は同一人の別session回答であり、独立聴取者の統計とは扱わない。

提示画面は無意味なIDと§4のT1三記述・「わからない」だけを出し、条件、版、case、SHA、対の相手を隠す。各試行で選んだ記述、回答確信、聞き取れない理由、再生回数を固定し、全回答確定後に鍵を開く。比較ごとに、主6対でT1-Iを選んだ件数、bodyと対照腕の対応差、foilと不明の件数、先行提示側のみの感度、各sine条件の回答を別々に出す。sineの無差判定は§0.1で取得前に無差を登録できた比較だけに適用し、接続除去のsine-flowへは適用しない。旧新版12対は方向を探索せず記述的に報告する。共同成功の数値（意図選択の最低件数、登録した負対照の一致、許容差、欠損時の判定、先行提示の扱い）は**未採用・未凍結**であり、観測前に登録する。関係仮説T1-Iは§4の「受け渡し」を維持し、聞いた後に付け替えない。

I12bのA1は二経路の個別切断と共同関係を扱う**別登録**とし、本書§8は受け取り欄に留める。T2の素材、対照、分母、提示とI12bとの版対応は、変調モデルと新版I11-2／I12bの登録・取得が定まってから別途固定する。旧T2の12本の結果を新版のT2 A1へ流用しない。

以下の§1〜§7・§9は2026-09-24に選んだ**旧60本案の履歴**である。新しい受入の仕様と衝突する場合は本節を優先し、旧手順を受入として実施しない。§10は歴史的な素材台帳として維持する。
