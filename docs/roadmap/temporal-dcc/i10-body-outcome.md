...
上表で「設計途中」とした比較量・尺度・窓・締切・不明時の規則は、2026-09-21に[I11-1](i11-onset-comparison.md)で登録した（T1 footprintは登録済み、T2 到来項は配送契約と量の形まで。上表の「到来期待」行の供給元は、`Engine` の直接参照ではなく共有snapshotに載せるCDF表へ改める）。
...
### Astra再々レビュー（commit `69e728f`）の結果（2026-09-21）

判定は「可」。二度目の指摘5件は全て解消と確認された。対照の停止点が学習経路へ入力する要素を止めていないこと、
追加照合した`temporal_observation`も差が資源計測欄だけであること、環境変数分岐が規則に抵触しないこと、
edgesの重複探索がhop経路に入らないことも確認された。新規指摘は低が1件で、対照scriptがon側でも親環境を
継承し、親に停止変数があると両側が停止する穴だった。on側で変数を除き、候補件数`on > 0`・`off == 0`を
assertする形へ直した。保存済みの結果はこの条件を満たしており、証拠への影響はない。
レビュー全文は`target/i10-bank-forecast-inputs-20260921/astra-review-69e728f.md`。

レビュアーの回答は「限定付きで監査表を満たしたと記載可」である。限定は、固定development素材への一致で
あること、未知素材への一般化・認知的妥当性・期限内の候補消費・R2／作者／実機受入を含まないこと。
レビュアーが再実行したのは対象2件の単体testと保存結果の再集計であり、全Rust再ビルド・全test再実行・
実音からの4,936対再計算・負荷再測定は行っていない。それらは本記録の同日の実行記録が担う。
...
狭い範囲のI10（milestones §2）は、三度目のレビュー（判定「可」）を経て、上の限定付きで監査表を満たした。
完了が意味するのは、実Voiceの身体・
