...
代理（ADSRの包絡）と実際のtoneの最大bin差は、modalでほぼ1.0に達する。これは少なくとも一つのbinで
正規化powerの差がほぼ最大に達したことを示し、16 binすべての形の違いを意味しない。この48件と`body`の
3,342件は別の発音機会なので、両分布の比較だけで同一条件における予測の優劣は判定できない
（2026-09-24の再レビューを受けて訂正）。件数は48/3,390（1.4%）である。
...
| §5.5 | APIの作用 (a) `body`対`proxy` | 未達（2026-09-24の再判定） | harmonic／modalの6条件で選択差とWAV相違を観測したが、同じD・同じ外部入力の条件を厳密には満たしていない。下の再レビューと再取得方針を参照 |
...
この時点では要件がそろったと整理したが、2026-09-24の最初の分岐の検査と再レビューにより、
**§5.5(a)は未達と判定し直した。技術完了は宣言しない。** ほかに判断の際に見ておく点を挙げておく。
...
上の残作業を、blancheの最終版（`72de44a`。`1a26d95`の返却drop解除と再送を含む）で実施した。
技術完了の宣言はしない。
...
ここから言えるのは、`power_k`の差とともに`D`のf32丸めと外部energyの微差が存在した、ということまでである。
当初の「選択を分けた入力は`power_k`」という主張は撤回する（2026-09-24の再レビュー）。powerの絶対差と
外部energyの相対差の桁を比べても、候補間の費用余裕や正規化を含む感度を考慮した因果分離にはならない。
§5.5(a)が登録で「同じ`D`」と書いた条件は、実装上`proxy`の`D`が別経路で作られるため厳密には満たされない。
...
## 再レビューと同一recordでの再取得方針（2026-09-24）

`a43cdf8`の本文全文と、§5.5(a)の微差を留保に書いた宣言文案をAstraへ渡して再レビューした。
判定は「不可」。文書だけのレビューであり、生の測定データの再検査ではない。
判定全文は`target/i11-stage1-rereview-20260924.md`に保存した。宣言文案は採用していない。

主要な未達は§5.5(a)である。残作業5件のうち最初の分岐の検査は、要件充足ではなく不一致を明らかにした。
§5.2の反事実検査は同じ観測でpowerだけを替えると選択が変わり得ることを示すが、別々の実行の最初の分岐と
WAV差の原因までを示すものではない。利用者は、同じD・同じ外部入力という登録条件を維持して修正・再取得する
方針を承認した。許容差を追加して従前の取得を合格扱いにする変更は行わない。

### 原因と実装

`PhonationEngine::footprint_hold`が`footprint = body`の場合にしか要求用のholdを返さず、`proxy`設定では
代表身体recordを要求していなかった。一方、`TemporalParticipation::candidate`は、recordがあればその整数
サンプル数のDを使い、無ければ`hold_sec + release_sec`のf32値からDを作っていた。

両方の`Some`設定で同じ代表recipeのrecordを要求し、recordを共有する同じ経路を通す。選択時にのみ、bodyは
recordのpower、proxyは同じD・16 binの位置で求めたADSR代理のpowerを使う。absent・stale・unsupportedの
状態選択と配送規則も共通経路に置く。`None`は引き続き要求を出さず、従来の64分割を使う。
proxy側のworker処理とrecord報告は増える。この変更は比較の対照を成立させる修正であり、proxyの処理負荷を
従前と同じとみなす根拠にはならない。

### 再取得前に固定する確認項目

1. 同じrecordを使うbody／proxyで、D・16 binの位置・外部energyが厳密に一致することを回帰検査する。
   offlineの要求・受領を含む比較で、proxyにもrecordが渡ることを確かめる。
2. 凍結済みの登録12条件を新しい取得先でbody／proxy／Noneのoffline renderとして実行する。
   最初に選択またはskipが分かれるまでの非power入力を厳密比較する。決定や候補の配列長、候補の有無、
   スケジュールの不一致を黙って除外しない。sineの分岐なしは許されるが、harmonic／modalで選択差とWAV差を
   再取得し、§5.5(a)を改めて判定する。
3. 同じ最終版のNone出力を、同一マシンで取得済みの基準版Noneと比較し、§5.4aを再確認する。
   そのreport対で§5.4bの共通候補recordも再比較する。従前の結果と新しい取得を混同しない。
4. 新しい取得、検査結果、ソースとbinaryの識別を記録した後で完了を再判定する。現時点の§5.5(a)は未達のまま。

### 再レビューの補足指摘への整理

- §5.4b。従前の取得が使った`65a0314`から`72de44a`までのソース変更は、`1a26d95`のfootprint返却dropの
  解除と再送である。候補energyの`project_window`、`evaluate`、`evaluate_scheduled`、`process_packet`と
  `energy/ratios.rs`は変わっていない。この系譜は計算内容を引き継ぐ根拠になるが、最終版の共通recordを再比較
  した実測ではない。上の再取得で後者も記録する。片側にしか無いrecordは件数として報告し、全件の欠落原因を
  特定できたとは述べない。
- §5.9。再投影の整数サンプル区間は`[onset, onset + ceil(D_rep))`である。`body`の3,342件と
  `proxy(stale)`の9件は整数サンプル数のrecordからDを受け取るので、登録のDと同じ終端になる。
  `proxy(absent)`の39件だけは端数を持つ可能性があり、16 binの境界にも丸めの差が入り得る。この39件の
  実際の端数件数・差の扱いと、代理分布の最大値を元の出力から補う。bodyの分布とこの未確認部分を分ける。
- §5.6と§5.7。返却dropの解除と再送の単体test、および待ちを除く処理負荷の改訂規則への適合は認められた。
  総hop負荷、A/A floorの統計的同等性、liveでの作用は引き続き主張しない。

### 修正後の手元検査と未実施の取得

`a43cdf8`を親とする作業差分で、`footprint_hold`のbody限定を外した。既存の単体testをproxyにもrecordを
要求する契約へ更新し、offline renderの回帰testをbodyの2回一致とbody／proxy比較に拡張した。
このmodalの回帰素材では、最初の選択またはskipの分岐まで、受領recordの識別・受領時刻、D、16点の位置、
候補の外部energyが厳密に一致した。分岐点のpowerと、実際のWAVに差があることも確認した。
この検査は登録12条件の再取得の代わりにはしない。

`locate_i11_first_divergence.py`は、最初の選択差だけでなくskip差、先行する非power入力の差、
スケジュールや候補配列の不一致も失敗として扱う。record識別・配送frame・sample rate・包絡入力・D・
16点の位置と外部energyを厳密比較する。登録全件の件数、分岐の有無、WAVとNone基準の確認を含む
[再取得手順](i11-inputs/README.md)を用意した。

手元のMacでの検査結果は次のとおり。

- 変更した単体testとoffline render回帰testは合格。
- PythonのI11検査4本は計24 test合格。Clippy（`-D warnings`）、format、差分形式検査も合格。
- 必須の`RUST_BACKTRACE=1 cargo test -- --nocapture`はexit 101。libは1,075成功・1失敗・36 ignoredで、
  後続のintegration suiteには進んでいない。失敗は`migrated_power_counterexample_and_rounding_boundaries`の
  `powf(2.0)`固定bit比較で、実測`0x400c740b6d82ad28`、期待`0x400c740b6d82ad27`の1 ULP差である。
  該当テストと計算経路のソースは親commitと一致し、今回変更したfootprint経路を呼ばない。親commitの
  別ビルドでの再実行はしていない。全文と終了状態は`test_report.txt`・`test_status.txt`に記録した。
