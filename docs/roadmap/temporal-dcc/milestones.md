...
**I8までの初期診断版の技術完了を維持。2026-09-13の著者判断によりI9を本体の必須工程から外し、長期の聴取応答を扱う研究拡張へ再設定した。次は対象機構と生成作用の対応を定める。**
...
I8初期診断版は技術完了。I9の自動継続は終了し、次はI10以降で使う対象機構と生成作用の対応を定める。
I9を待つ依存は設けない。R/Aの収集・適合・可聴性・作者採用・全負荷検査は未完了。
既存変更と今回の変更は未commitのまま保持する。
