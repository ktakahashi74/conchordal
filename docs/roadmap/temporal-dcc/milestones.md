...
2026-09-21: 狭い範囲のI10の完了判定は保留（Astraレビューは`1a42001`・`2dc41af`とも「不可」。支持境界、学習不変の対照、hop経路負荷、gate終端、折れ点容量、2 sample spanを同日に修正済みで、二度目の修正への再々レビューは未実施）。harmonic／modal身体を搬送波laneとして候補energy予測へ接続し、旧近似を廃した。調整に用いた固定development素材では、変更していない符号判定基準（epsilonなし）で71分岐・両bus・4窓の4,936対すべてが実音と一致した（反転0・見落とし0・捏造0）。Python照合は宣言済みlaneと近似に対する積分の整合を確認したもので、近似自体の独立検証ではない。窓平均の二乗誤差は1.77e-5から1.1e-13。中間版に残った近接対22の反転は、予測modelの厳密評価により求積誤差と特定し、envelope折れ点での分割、attack／decay区間の細分、span内envelopeの局所指数の繰り込みで解消した。細分度はこの素材上で調整した値であり盲検ではない。[記録](i10-body-outcome.md#harmonicmodal身体の搬送波予測を候補energyへ接続2026-09-21)。
...
I10完了は作者採用（A1／A2）・資源受入（R2）・実機受入（A3）を含まない。次の一単位は再々レビューによるI10完了判定の確定、その後にI11の実装前登録（比較量、窓、締切、unknown時の規則）。
