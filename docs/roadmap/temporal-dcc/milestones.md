...
<a id="two-tier-terrain-scope"></a>

#### 2026-09-24追記：二層化と時間の心理音響地形を含む対象版

作者採用済みの[台帳 `two-tier-rule`](../../design-notes/technote-ledger.ja.md#two-tier-rule)に従い、本体へ置く機能の三条件と、その計算を原理層／作者規則層へ分類する規則を分ける。既存文献の定量モデルは原典との対応を確認した部分だけを原理層とし、独自の近似・閾値・結合係数・学習則は作者規則層に置く。学習済みの重みは観測に従う状態であり、適合値や認知の定数と取り違えない。
...
| I11 一機構の局所生成 | 支持された関係一つ→実消費者→音響差を示す。入力一致、状態介入、期限、off基準、APIの作用を検査する | 対象I、対応するR4・R5の合格（§1.3以降の本体では契約検査O03／O09とA1に置き換える） | M6。対象を明記して一機構ずつ閉じる。最初の一機構の登録は[I11-1](i11-onset-comparison.md) |
...
2026-09-21: 狭い範囲のI10を、固定development素材への限定付きで完了とした（Astraレビューは`1a42001`・`2dc41af`が「不可」、修正後の`69e728f`が「可」。支持境界、学習不変の対照、hop経路負荷、gate終端、折れ点容量、2 sample spanを同日に修正）。harmonic／modal身体を搬送波laneとして候補energy予測へ接続し、旧近似を廃した。調整に用いた固定development素材では、変更していない符号判定基準（epsilonなし）で71分岐・両bus・4窓の4,936対すべてが実音と一致した（反転0・見落とし0・捏造0）。Python照合は宣言済みlaneと近似に対する積分の整合を確認したもので、近似自体の独立検証ではない。窓平均の二乗誤差は1.77e-5から1.1e-13。中間版に残った近接対22の反転は、予測modelの厳密評価により求積誤差と特定し、envelope折れ点での分割、attack／decay区間の細分、span内envelopeの局所指数の繰り込みで解消した。細分度はこの素材上で調整した値であり盲検ではない。[記録](i10-body-outcome.md#harmonicmodal身体の搬送波予測を候補energyへ接続2026-09-21)。
同日、release・harmonic／modal身体・両busへの展開を12条件で検査し、音声が`off`と一致すること、学習recordが再実行間で一致すること、候補bankだけを止めた対照と学習内容が一致することを確認した。縮小した負荷も4／16／64 Voiceで測定し、候補workerの飽和（flow条件で要求の55〜98%が容量drop、1 record約7／80〜210／60〜130 ms）と64 Voiceのhop予算超過を失敗として保持したままR2へ引き渡した。[記録](i10-body-outcome.md#release任意身体両busの検査と縮小した負荷の資源引渡し2026-09-21)。
I10完了は作者採用（A1／A2）・資源受入（R2）・実機受入（A3）を含まない。同日、I11の最初の一機構（onset-now／delayed onset比較を`temporal_participation`へ返す経路）の実装前登録を[I11-1](i11-onset-comparison.md)に記した（Astraレビュー4往復。初稿〜第3稿は「不可」、第4稿は「条件付き可」で、条件2件を第5稿へ反映）。第1段が返すのは代表onset条件付きの身体予測であり、実kick・実seedに条件付けたI10予測そのものではない。T1の自声footprint（第1段）は比較量・集計・有界窓・四つの時刻と締切・不明時の規則・off基準（`[temporal_onset_comparison]`既定`None`でbit一致）・検査計画まで登録済み。T2の到来項（第2段）はCDF表の配送契約と量の形まで登録し、Hazard設定の凍結値とCDF格子の最終値は第2段の実装前に追記する。資源対策はR2に残す判断（2026-09-21）。2026-09-22にI11第1段を実装した（`[temporal_onset_comparison]`、footprint核と専用queue、Voice側の識別・要求・状態選択、report 8欄）。全Rust 1,154成功、clippy通過。`None` は基準`32a6389`とWAV・学習recordが12条件でbit一致（新8欄を除く）。`body` の実走で経路の稼働を確認。同日、§5.5(a)と§5.7を取得した。`body` 対 `proxy` の選択差はharmonic／modalの6条件で観測（WAV相違）、sineでは0。§5.7は登録許容（±5%、1 ms、超過同数）で16組中3組のみ全欄合格。`population_us` の増分は絶対値5〜9 µs域で相対許容が厳しい。置換済み要求が56〜88%と高い。第1段の技術完了は未判定。[記録](i11-onset-comparison.md#第1段の測定2026-09-22と)。同日、置換済み比率の原因（entrained包絡の現在値と発音中の振幅がrecipeの識別に入り、毎hop変わっていた）を特定し、識別規則を改訂した（modulatorの代表化と代表振幅1.0。footprintの値は丸め差を除き変わらない）。再現条件で置換済みは86〜99%から0〜18%へ下がった。[記録](i11-onset-comparison.md#識別規則の改訂2026-09-22)。§5.5・§5.7の値は改訂前の実装のもの。2026-09-23、改訂commit`77e07b6`のbinaryで§5.5(a)・§5.7を登録範囲（12条件のrender、4／16 Voice 8条件のinstrument）で再取得した。置換済み要求は登録8条件で0〜25%へ下がり（旧56〜88%）、§5.7の全欄合格は7/16組（旧3/16）。§5.5(a)はharmonic／modalでT1固有の作用が維持（sineでは不成立のまま）。第1段の技術完了はまだ未判定。[記録](i11-onset-comparison.md#第1段の再取得2026-09-23識別規則改訂後の55と57)。同日さらに、§5.7の許容にA/A（none対none）雑音幅の下限を加える規則を、`body`対`none`の取り直しの前に登録した（中央値・p99は`max(0.05×none, floor)`、最大は`max(1 ms, floor)`、超過hop数は差がfloor以内。floorは16組のA/A差の最大値）。A/A自体が元の許容で6/16組落ち、`population_us`と超過hop数同数の規則が雑音幅より狭いことを確認した。[登録](i11-onset-comparison.md#57の許容にaa雑音幅の下限を加える2026-09-23取得前の登録)。取り直しの結果は元の許容で6/16組、A/A規則で8/16組が全欄合格。`population_us`中央値は16組すべて合格となり（相対最大+21.4%、絶対差1.23 µs＜floor 1.27 µs）、識別hashと要求送出が中央値へ乗せる量は、改訂した受入規則（A/A floor）に適合した（雑音と区別できないことの証明ではない。2026-09-23訂正）。残る実質的な問題はmodal-flow-4の`elapsed_us`最大（1 msの1.8〜2.0倍、独立2回の取得4組すべてで超過、同組のA/A雑音の約6倍）。sine-holdはfootprint要求0件のまま`synthesis_us`が落ちており、1回分のA/Aから取ったfloorが一部の欄で雑音幅を下回っている。[記録](i11-onset-comparison.md#57の取り直しと二つの許容による判定2026-09-23)。第1段の技術完了は未判定。同日、§4.6に登録済みで未実装だった各項の報告（`participation_decision`・`body_footprint`）と独立参照`scripts/verify_i11_stage1.py`を加え、再現条件の6本のreportで§5.1〜5.3の全検査が通った。§5.2で選択が変わる決定はsine 0、harmonic 19／102、modal 37／107。`body`のrenderはfootprintの配送が実時間に依存するため再実行で同じにならないことも分かった。[記録](i11-onset-comparison.md#独立参照による入力一致状態介入期限の検査2026-09-23)。次の一単位は、modal-flow-4の`elapsed_us`最大の超過箇所をprofileから特定すること、A/Aを複数回に増やしてfloorを分布で置き直すこと、§5.7のreport有り8組の取り直し、登録12条件での§5.1〜5.3、§5.9のoffline再投影。同日、offline renderに限りfootprintを要求と同じhopで全件受け取る決定的な配送を登録・実装し、`body`のrenderが再実行で一致するようにした（instrumentは実時間の配送のまま）。[記録](i11-onset-comparison.md#offline-renderでのfootprint配送の決定化2026-09-23登録)。これに伴い§5.5(a)も取り直す。同日、登録12条件での§5.1〜5.3を`aebae2b`で実施し、24本のreportで22種の検査が全通過（失敗0。`proxy(absent)`・`proxy(stale)`の分岐も通り、skipと`proxy(unsupported)`は12条件でも現れない）。§5.2で選択が変わる決定は121／3,346（sine 0、harmonic 1／56・6／205・7／846、modal 22／56・38／204・47／856）。sine-holdは決定0件のため、独立参照が空reportの番人として終了コード1を返す。続いて`a6637db`で§5.5(a)を取り直した。先にmodal-flow-4で`body` renderを2回行い、WAVのSHA-256一致と、`onset`・`participation_decision`・`participation_context`・`participation_outcome`を含む24種のrecordの完全一致を確認した（差は`computed_at`と実時間欄、および非同期candidate workerの`body_candidate_energy`のみ）。12条件の選択差は`77e07b6`の実時間配送と全条件で一致し（`t1_effect_observed` 6/12）、変わったのはfootprint源の内訳である（`proxy(absent)`はharmonicの2条件から消え、`stale`は64 Voiceで11→2）。独立参照も`a6637db`の24本で3,558,101件すべて通過。[記録](i11-onset-comparison.md#55aの取り直し2026-09-23a6637dbの決定的配送)。第1段の技術完了は未判定。続いて、取得前に登録した規則どおりA/A 3 pass（48標本）からfloorを置き直し、§5.7の16組を三つの許容で判定した。中央値・p99の6欄は1 pass floorより大きくなり（`synthesis_us` p99は73.5→223.6 µs）、1回分では雑音幅を下回るという診断が裏づけられた。収束確認ではpass1〜2のfloorをpass3に当てて160件中4件が超過し、3 passでもまだ完全には収束していない（floorはこの数字で調整していない）。合格は元の許容で5/16、1 pass floorで8/16、3 pass floorで11/16。残る5組はいずれもfloorが効かず、5%か1 msが許容を決めている。`population_us`中央値は15/16合格で、唯一落ちたharmonic-flow-16 reportは`none` 119.65 µsに対し+8.81 µs（+7.4%）と基準が大きく、同条件のno-reportは合格するため、report有りの`body`側にだけ乗る§4.6の決定報告の費用が最も素直な説明になる。modal-flow-4の`elapsed_us`最大の超過hopは6実行すべてframe 401で、増えている相は`analysis_wait_us`のみ（body 7770.8 µs対`none` 1512.9 µs、`population_us`は6.5 µs）。footprint要求は8件ともframe 0〜1で完結しており、frame 401とは時間的に重ならない。`analysis_wait`が3 msを超えるhopは両変種ともframe ≡ 1 (mod 10) に限られ（body 23件・`none` 30件、中央値とp99は一致）、最大のspikeはその周期hopの直前にonsetが入ったときに出る（bodyは3反復ともhop 400にonset、`none`は395〜401にonsetなし）。走行全体の`elapsed_us`合計と予算超過hop数には差がない。原因の候補は、分析待ちの外れ値を`max`が拾っていること（最も支持される）と、T1がonsetを動かした結果として分析が実際に重くなったこと（除外できない）で、候補選択とfootprintの経路は除外した。修正はしていない。[記録](i11-onset-comparison.md#57を三つの許容で判定する2026-09-23aa-3-pass-floor)。第1段の技術完了は未判定。同日、report付きの`population_us`の増分は決定報告の書き出しが集団更新の中にあったためと分かり、報告の相へ移した（`c41fdb3`）。`elapsed_us`は解析待ちを除いた`own_us`で判定する規則を取得前に登録した（[記録](i11-onset-comparison.md#57のelapsed_usをhop自身の仕事で判定する2026-09-23取得前の登録)）。同日、§5.9のoffline再投影の入口（ignored test `acquire_representative_gap` と`scripts/summarize_i11_representative_gap.py`）を作り、再現条件で分布を出した（`power_gap` 最大2.2e-2、`term_gap` 最大7.0e-3）。[記録](i11-onset-comparison.md#59の入口と再現条件での分布2026-09-23)。同日、登録入力一式（12条件の`.rhai`、3つのconfig、`plan.json`、取得と判定のscript、§5.9用`registration.json`）を`docs/roadmap/temporal-dcc/i11-inputs/`へ入れた（`fe42afc`）。`inputs_sha256`の値は09-22の凍結のままでキーだけ向け直し、複写との一致を検証した。scriptはパス処理だけを直し（リポジトリ根を`Cargo.toml`探索で決める、`--plan`・`--inputs`・`--floor`）、既存データに対して移設前と同じ出力が出ることを確認した。続いて`c41fdb3`と`6f85f42`の後の`029795c`で§5.7を16組取り直した（リポジトリ化した入力からの初回取得、96実行・失敗0）。`own_us`のfloorは既存のA/A 3 passから取得前に算出した（中央値107.020／p99 156.901／最大552.433 µs、超過hop 1。待ちを引くと中央値とp99は生の`elapsed_us`の171.471／244.752 µsより下がる）。**登録の組み合わせ（`population_us`・`synthesis_us`は3 pass floor、`elapsed_us`の3欄と超過hopは`own_us`）で16組すべてが全欄合格した（落ちた欄0）。§5.7が全欄で通ったのは初めてである。** 参考として生の`elapsed_us`のままなら13/16、元の登録許容なら8/16。許容に最も近いのは`own_us` p99のsine-flow-16 reportで7割。`c41fdb3`の効果も実測で確かめた——harmonic-flow-16 reportの`population_us`中央値は、修正前が差8.81 µs（+7.4%、許容5.98）で不合格、修正後は差3.40 µs（+2.9%、許容5.90）で合格。report有りの`body`側にだけ乗っていた費用が決定traceのJSON符号化だったという09-23の推定が裏づけられた。生の`elapsed_us`で落ちる3組（modal-flow-4のreport／no-reportの最大、modal-flow-16 reportのp99）では、同じ組の`analysis_wait_us`最大の差が生の差を上回り（1.59〜1.71 msに対し1.9〜2.0 ms）、待ちを引くと`own_us`中央値の差は8.8 µsと2.6 µsになる。`analysis_wait_us`の中央値の差は組によって向きが揃わず、`listener_wait_us`は差がない。[記録](i11-onset-comparison.md#57が全欄合格する2026-09-23own_us判定と決定報告の修正の後)。第1段の技術完了は未判定（§5.9の登録12条件と§5.4bが未実施）。同日、§5.9を登録12条件で取得した（`registration.json`の12条件、ignored test で192.6秒、集計は`scripts/summarize_i11_representative_gap.py`）。投影できたonsetは3,390件、`coherent_energies`に`None`を含む除外は0件。`no_decision` 84件はsine-holdの3条件、`overlap_not_applied` 39件は`proxy(absent)`と一致する（各Voiceの最初の決定）。`body` 3,342件の`power_gap`中央値は素材で大きく違い、modal 2.3e-3、sine 2.7e-2、harmonic 8.7e-2（最大1.34e-1）。`term_gap`は最大1.29e-2。再現条件（最大2.2e-2／7.0e-3）より登録12条件のほうが厳しい。代理に落ちた48件（1.4%）では最大bin差がmodalでほぼ1.0に達した。ただし`body`の3,342件とは発音機会が異なるため、この分布比較だけでは同条件での優劣は判定しない。`proxy(stale)` 9件の`overlap_gap`・`term_gap`が0なのは外部energyが全ゼロの窓だったためであり、powerの一致を意味しない。合否は付けず分布の報告とし、A1の材料とする。[記録](i11-onset-comparison.md#59を登録12条件で取得する2026-09-23)。骨組みだった`i11-inputs/representative_gap.py`は削除し、READMEを ignored test と集計scriptへ向け直した。第1段の技術完了は未判定（§5.4bの凍結packet再評価が未実施）。同日、§5.4bを凍結packetの再評価から、`None`での基準版と新版の対の再実行で候補recordを突き合わせる方法へ改める登録を、取得前に置いた（[登録](i11-onset-comparison.md#54bを対の再実行で確かめる2026-09-23取得前の登録)）。同日、その登録どおり§5.4bを実施した。基準タグ`i11-stage1-baseline`は、指示の`32a6389`がHEADの祖先でなく固有履歴18件を伴うと分かったため、同じ登録commitで本流にある`103a32f`へ付けた。`src/` tree・`Cargo.toml`・`Cargo.lock`が一致し（差分は`web/static/pdf/`のPDF 1件のみでビルドは読まない）、そこからビルドした`conchordal-render`のSHA-256が`plan.json`記録の基準binaryとバイト単位で一致することを確認した。remoteへはタグだけをpushした。登録12条件を`None`・offline render・report付きで基準版と新版の両方で実行し、`body_candidate_energy`を6つのキーで突き合わせて`processing_us`以外の全欄をbit比較した結果、**突き合わせた11,421件すべてが一致（不一致0件）、突き合わせ0件の条件も無く§5.4bは合格**。片側のみのrecord（基準156件・新144件）は登録どおり飽和による採否の揺れとして件数だけ数え、sine-holdの3条件では揺れが0件だった。[記録](i11-onset-comparison.md#54bの取得と判定2026-09-23)。この時点では§6の要件がそろったと整理したが、2026-09-24の再レビューで§5.5(a)を未達と判定し直した。**技術完了は宣言しない**。判断時に見る点を記録へ併記した——§5.6は登録12条件の実走ではなく単体testで満たしていること（skipと`proxy(unsupported)`は12条件に現れない）、§5.7の合格は`elapsed_us`を`own_us`で判定する改訂登録に依り生の`elapsed_us`では13/16であること、§5.9は分布の報告で代理48件の`power_gap`がmodalでほぼ1.0であること、§5.5(a)はsineでは成立しないこと。生成作用の採用（A1／A2）は別で、既定は`None`のままである。同日、第1段の技術完了の宣言文案をAstraに問い、「不可」を受けた（§5.7の範囲、A/Aの解釈、§5.4bの同等性、§5.5の説明、§5.6の返却drop、§5.9の分布と母集団）。§6を改め、§5.7は待ちを除く処理負荷に限り、総hop負荷はR2へ回した。§5.4bは共通recordの内容一致に改め、凍結packetの再評価は未実施と残した。§5.6の返却dropの解除と再送を実装した。[記録](i11-onset-comparison.md#第1段の完了判定へのレビューと対応2026-09-23)。2026-09-24、その残作業5件を最終版（`72de44a`）で実施した。§5.4aはタグ`i11-stage1-baseline`からbuildした基準binary（render `3614d998…`）と最終版（`af11547c…`）で登録12条件を`None`・render・report付きで実行し、**12条件すべて合格**（WAVのSHA-256一致、学習record 10種一致。実時間欄と第1段が加えた`participation_context`の8欄を除く。候補recordの件数差は飽和の揺れとして別集計）。§5.7の最終取得には、登録した1 pass floorでの判定（11/16。元の許容8/16と3 pass floorの13/16の間）、待ちの分布（`analysis_wait_us`は中央値1,260前後で差の向きが組で揃わず、`listener_wait_us`は系統差なし）、返却遅延のp99・最大（sine-flow 0.0 ms、modal-flow-4 32.0 ms、harmonic-flow-16 106.7 ms。hop単位の離散値を取るためp99と最大が一致する条件が多い）を補った。§5.5(a)は`547b11d`の取得に`locate_i11_first_divergence.py`をかけ、sineの6条件は分岐せず、harmonic/modalの6条件がindex 5〜31で分岐した。**ただし「違いが`power_k`だけ」は文字どおりには成り立たない**——分岐した6条件すべてで`footprint_d_samples`（86400.0対86399.9977、0.0023サンプル＝f32の丸め）と、その窓がもたらす`candidates.external_energy`（相対差1.3e-7〜7.1e-7）も併せて違う。他の決定入力と候補の`at`は全一致で、同じ決定の`power_k`の差0.84〜1.00は外部energyの差より6桁大きい。ただし、差の大きさだけでは選択原因を分離できず、「選択を分けた入力は`power_k`」という主張は再レビューで撤回した。同じD・同じ外部入力という登録条件は未達である。§5.5(b)は最終版の`body`対`none`で測り、`body`対`proxy`では差の出ないsineにも差が出ることを確認した（分割数と打ち切りの違いを含むため。参考記録）。§5.9の表は`overlap_gap`・`term_gap`の中央値・p95を加え、`body` 3,342件と代理48件を別母集団に分けた（`proxy(absent)` 39件は重なり項が適用されず0件）。再現性確認の型数の矛盾は数え直して**30種**（24一致＋6相違）と訂正した。[記録](i11-onset-comparison.md#レビューの残作業5件を行う2026-09-24)。技術完了の宣言はしていない。Astraの再レビューは不可。利用者の承認に基づき、登録条件を維持したままproxyにも同じ代表recordを配送する修正を実装した。手元の実配送回帰検査は合格し、全体テストでは変更経路外の固定bit検査に1 ULP差が残る。次の一単位は、同じrecord・D・外部入力での登録12条件の再取得と、修正後Noneの§5.4a／b再確認。結果がそろうまで宣言文は採用しない。[再取得方針](i11-onset-comparison.md#再レビューと同一recordでの再取得方針2026-09-24)。ほかに、A/Aのpass数を増やしてfloorの収束を確かめること、代理の`power_gap`が大きい点をA1の聴取材料としてどう扱うかの整理。
...
I11第1段（T1 footprint）の比較量・有界窓・締切・欠測時の選択規則は[I11-1](i11-onset-comparison.md)で登録した（2026-09-21）。第2段（T2 到来項）は配送契約と量の形まで登録済みで、Hazard設定の凍結とCDF格子、I12b到来経路の更新則・尺度は第2段の実装前登録で確定する。登録は機能成立の主張ではない。
...
<a id="terrain-handoff-20260924"></a>

### 2026-09-24：二層化採用後の引継ぎ単位

§1.3への追記を、新しいT1／T2作業に先立って行った。対象はT1・T2の心理音響地形の登録準備、I13、R2／A1の改訂である。今回の終了点は、担当Iの番号案、旧60本の扱い、T1マスキングモデルの候補と推奨を、根拠と独立レビューを付けて作者へ提示するところである。担当I案はI11-3／I11-4であり、番号とモデルの選択は未承認、登録は未凍結である。

I12bの保存記録と結果記録は未取得・技術完了未判定である。現在の稼働を保存記録だけで推定せず、封印sourceを維持する。文書の準備を並行し、source変更はI12b終結・基準版固定後の別worktreeと別`CARGO_TARGET_DIR`で行う。時間計測中にcargo・テスト・renderを競合させない。

既存の順序は、I12bの取得・作業終結と基準版固定 → I4第一段階のbit保存分離 → 第二段階と影響するI10の再検証 → 正式なR2、である。地形の変更に依存しないI13の骨組み・R2計器・A1の道具を準備し、T1地形とそのA1、T2地形と新しいI11-2／I12b比較・T2のA1を追加する。R2の本取得は、I4／I10の条件に加えて両地形と影響回帰を満たした最終構成を待つ。
