...
2026-09-21: 狭い範囲のI10の監査表を満たした（Astraレビュー`1a42001`は「不可」。指摘の支持境界・学習不変の対照・hop経路負荷を同日に修正済みで、修正後の再レビューは未実施）。harmonic／modal身体を搬送波laneとして候補energy予測へ接続し、旧近似を廃した。調整に用いた固定development素材では、変更していない符号判定基準（epsilonなし）で71分岐・両bus・4窓の4,936対すべてが実音と一致した（反転0・見落とし0・捏造0）。Python照合は宣言済みlaneと近似に対する積分の整合を確認したもので、近似自体の独立検証ではない。窓平均の二乗誤差は1.77e-5から1.1e-13。中間版に残った近接対22の反転は、予測modelの厳密評価により求積誤差と特定し、envelope折れ点での分割、attack／decay区間の細分、span内envelopeの局所指数の繰り込みで解消した。細分度はこの素材上で調整した値であり盲検ではない。[記録](i10-body-outcome.md#harmonicmodal身体の搬送波予測を候補energyへ接続2026-09-21)。
同日、release・harmonic／modal身体・両busへの展開を12条件で検査し、音声と学習recordの不変を確認した。縮小した負荷も4／16／64 Voiceで測定し、候補workerの飽和（flow条件で要求の55〜98%が容量drop、1 record約7／80〜210／60〜130 ms）と64 Voiceのhop予算超過を失敗として保持したままR2へ引き渡した。[記録](i10-body-outcome.md#release任意身体両busの検査と縮小した負荷の資源引渡し2026-09-21)。
