...
- **第二段階後を主判定にする方針（選択済み）**：I12bの取得・作業終結と基準版の固定 → I4第一段階のbit回帰 → 第二段階の新規則登録・実装 → 研究検索の配置判断とI10／影響する生成経路の再検証 → 最終処理目録・不足計器の検証とR2登録 → R2 → A3の最終受入・A4監査。R2の正式取得はI10再検証まで待つ。R2合格をI4実装の着手条件にはしない。
