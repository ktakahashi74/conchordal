...
fn ordinary_acoustic_recurrence_reports_public_within_timing_without_private_actions() {
    ...
    let mut learned_pairs: f64 = 0.;
    let mut groove_bytes = None;
    for step in 1..=720 {
        let end = step * 512;
        model
        ...
        for g in snapshot.groups.iter().flatten() {
            let s = g.groove.unwrap();
            assert_eq!(s.end_sample, end);
            assert_eq!(*groove_bytes.get_or_insert(s.owned_bytes), s.owned_bytes);
            density_frames += usize::from(s.raw[42..50].iter().any(Option::is_some));
            grouping_frames += usize::from(s.raw[53].is_some());
            learned_pairs = learned_pairs.max(s.learned_pair_mass);
            ...
            assert!(s.raw.iter().flatten().all(|x| x.is_finite()));
        }
    ...
    assert!(
        density_frames > 0 && grouping_frames > 0 && learned_pairs > 0.,
        "density={density_frames} grouping={grouping_frames} pairs={learned_pairs}"
    );
    let before = serde_json::to_value(model.diagnostics().unwrap().timing).unwrap();
    let groove_before = serde_json::to_value(
        model
            .diagnostics()
            .unwrap()
            .groups
            .map(|g| g.and_then(|g| g.groove)),
    )
    .unwrap();
    ...
    println!(
        "public groove ordinary recurrence: density_frames={density_frames} grouping_frames={grouping_frames} learned_pairs={learned_pairs} owned_bytes_per_group={}",
        groove_bytes.unwrap()
    );
    println!(
...
#[test]
fn configured_groove_heads_consume_normal_acoustic_features_without_changing_evidence() {
    use crate::config::{
        ArrivalModel, TemporalGrooveConfig, TemporalGrooveHeadConfig, TemporalPeriodConfig,
    };
    use crate::temporal_cognition::{action_profiles, body_model, gesture, phrase};
    let mut head = TemporalGrooveHeadConfig {
        coefficients: [0.; 109],
        cutpoints: [-1., -0.2, 0.2, 1.],
        prior: [0.2; 5],
        temperature: 1.,
    };
    head.coefficients[85] = 0.25;
    head.coefficients[60] = 0.4;
    let mut opposite = head;
    opposite.coefficients[85] = -0.25;
    opposite.coefficients[60] = -0.4;
    let heads = TemporalGrooveConfig {
        means: [0.; 54],
        deviations: [1.; 54],
        groove: head,
        desire: opposite,
    ...
    let mut enabled = Recurrence::configured(
    ...
    .unwrap();
    let mut disabled =
        Recurrence::configured(Frontend::new(space(), config(0)).unwrap(), period, None).unwrap();
    let mut scan = vec![0.; space().n_bins()];
    scan[20] = 1.;
    let mut supported = 0;
    let mut different = 0;
    let mut density_windows = 0;
    let mut timing_coordinates = 0;
    let mut context_coordinates = 0;
    let mut context_positive_cells = 0;
    let profiles = action_profiles::tests::timing_pulse_model();
    let mut timing_scratch = None;
    ...
    .unwrap();
    let mut phrase = phrase::Phrase::new(0, 2, 0, 48000, 512, phrase::tests::config()).unwrap();
    for step in 1..=720 {
        let end = step * 512;
        let output = enabled
            .advance(end, end, observed(end, &scan, pulse(step)))
            .unwrap();
        let acoustic = enabled.acoustic_snapshot(&output);
        gesture.advance(&acoustic, &output.ridges, end).unwrap();
        phrase
            .advance(&acoustic, &gesture, None, enabled.diagnostics(), end)
            .unwrap();
        disabled
            .advance(end, end, observed(end, &scan, pulse(step)))
            .unwrap();
        let snapshot = enabled.diagnostics().unwrap();
        if context_positive_cells == 0
            && let Some(g) = snapshot
                .groups
                .iter()
                .flatten()
                .find(|g| g.active && g.groove.is_some_and(|s| s.raw[53].is_some()))
        {
            let mut shared = body_model::Shared {
                ...
                epoch: 2,
                end_sample: end,
                descriptors: phrase.body_descriptors(),
                assignments: [None; 8],
            };
            shared.assignments[0] = Some(body_model::Assignment {
                key: (g.ledger.group.epoch, g.ledger.group.generation),
                distance: 0.,
                common_coordinates: 6,
            });
            let mut table = action_profiles::Table::new();
            let table = table
                .refresh(&profiles, &phrase, &shared, &gesture, Some(&enabled))
                .unwrap();
            for cell in table.latest.into_iter().flatten() {
                let context = cell.groove_context.unwrap();
                if cell.evaluation_at == end {
                    assert_eq!(
                        context.values.map(|v| v.value()),
                        g.groove.unwrap().raw[50..54]
                    );
                    context_positive_cells += usize::from(context.values[3].value().is_some());
                }
            }
        }
        for g in snapshot.groups.iter().flatten().filter(|g| g.active) {
            let context = enabled
                .context_projection(g.ledger.group, end, end)
                .unwrap();
            assert_eq!(
                context.values.map(|v| v.value()),
                g.groove.unwrap().raw[50..54]
            );
            context_coordinates += 4;
            let timing = enabled
                .timing_projection(g.ledger.group, end, end, &mut timing_scratch)
                .unwrap()
                .finish();
            for (actual, expected) in timing
                .values
                .iter()
                .flatten()
                .zip(&g.groove.unwrap().raw[..42])
            {
                assert_eq!(actual.value(), *expected);
                timing_coordinates += 1;
            }
            let mut density = enabled
                .density_projection(g.ledger.group, end, end)
                .unwrap();
            density.finish(48000);
            for (i, value) in density.values.into_iter().enumerate() {
                assert_eq!(value.value(), g.groove.unwrap().raw[42 + i]);
                density_windows += 1;
            }
            assert!(
                disabled
            ...
            assert!(
                enabled
                    .density_projection(g.ledger.group, end + 1, end + 1)
                    .is_none()
            );
            let mut retired = g.ledger.group;
            retired.generation = u64::MAX;
            assert!(enabled.density_projection(retired, end, end).is_none());
        }
        let scores = snapshot.groove_heads.unwrap();
        assert_eq!(scores.window[1], end);
        assert_eq!(scores.errors, [None; 2]);
        let groove = scores.groove.unwrap();
        let desire = scores.desire.unwrap();
        assert_eq!(groove.observed_coverage, 1.);
        supported += usize::from(groove.reported_support_mass > 0.);
        different += usize::from(groove.expected_rating - desire.expected_rating > 1e-8);
        let mut new = serde_json::to_value(snapshot).unwrap();
        new.as_object_mut().unwrap().remove("groove_heads");
        assert_eq!(
            new,
            serde_json::to_value(disabled.diagnostics().unwrap()).unwrap()
        );
    }
    assert!(
        supported > 0 && different > 0,
        "supported={supported} different={different}"
    );
    assert!(density_windows > 0);
    assert!(context_positive_cells > 0);
    let timing_issue = enabled
        .diagnostics()
        .unwrap()
        .timing
        .unwrap()
        ...
        .into_iter()
        .flatten()
        .find(|g| g.within.supported)
        .unwrap();
    let group = timing_issue.group;
    let issue = 720 * 512;
    // This fixture fixes the association; it does not claim descriptor-transfer validity.
    let mut shared = body_model::Shared {
        model_version: profiles.body_model_version,
        bus: 0,
        epoch: 2,
    ...
    shared.assignments[0] = Some(body_model::Assignment {
        key: (group.epoch, group.generation),
        distance: 0.,
        common_coordinates: 6,
    });
    let observed_before = serde_json::to_value(enabled.diagnostics()).unwrap();
    let phrase_before = serde_json::to_value(phrase.snapshot()).unwrap();
    let mut table = action_profiles::Table::new();
    let snapshot = table
        .refresh(&profiles, &phrase, &shared, &gesture, Some(&enabled))
        .unwrap();
    let publication = table.publication(&profiles).unwrap();
    let frozen = enabled.groove_issue(issue).unwrap();
    let base = frozen.snapshot;
    let binding = action_profiles::consumer::Binding {
        source_id: 7,
        source_generation: 2,
        body_generation: 4,
        bus: 0,
    ...
    let body_state = crate::life::action_candidates::BodyState {
        permits_action: Some(true),
        active_at_candidate: Some(true),
        pending_opportunity: true,
        due_unconsumed: true,
    };
    let default = crate::life::action_candidates::Class::Continue
        .input(issue, issue, Some(24000), 48000, body_state)
        .unwrap();
    let mut projected_ratings = 0;
    let mut changed_ratings = 0;
    let mut consumer_ratings = 0;
    let mut positive_cells = 0;
    let mut new_intervals = 0;
    let mut changed_coordinates = 0;
    for cell in snapshot.latest.into_iter().flatten() {
        let prediction = cell.groove_heads.unwrap();
        assert_eq!(prediction.scope, "conditional_acoustic_base_heads");
        assert_eq!(
            prediction.observed_inputs
                + prediction.projected_inputs
                + prediction.unsupported_inputs,
            54
        );
        assert_eq!(prediction.issue_window, base.window);
        let mut bad = cell;
        bad.issued_at += 1;
        assert!(frozen.project(&bad).is_none());
        bad = cell;
        bad.evaluation_at = issue - 1;
        assert!(frozen.project(&bad).is_none());
        bad.evaluation_at = issue + 4 * 48000 + 1;
        assert!(frozen.project(&bad).is_none());
        bad = cell;
        bad.group.generation = u64::MAX;
        assert!(frozen.project(&bad).is_none());
        bad = cell;
        bad.groove_context.as_mut().unwrap().evaluated_at += 1;
        assert!(frozen.project(&bad).is_none());
        for (candidate, observed) in [
            prediction.groove.zip(base.groove),
            prediction.desire.zip(base.desire),
        ]
        .into_iter()
        .flatten()
        {
            assert_eq!(candidate.observed_coverage, observed.observed_coverage);
            if prediction.projected_inputs > 0 && candidate.reported_support_mass > 0. {
                projected_ratings += 1;
                changed_ratings += usize::from(
                    (candidate.expected_rating - observed.expected_rating).abs() > 1e-8,
                );
            }
        }
        if let Some(input) = cell
            .class
            .input(issue, cell.action_at, Some(24000), 48000, body_state)
            && let action_profiles::consumer::Pair::Paired { candidate, .. } =
                publication.pair(Some(binding), (7, 2, Some(4)), 0, issue, input, default)
        {
            assert_eq!(
                serde_json::to_value(candidate.groove_heads).unwrap(),
                serde_json::to_value(cell.groove_heads).unwrap()
            );
            consumer_ratings += 1;
        }
        let timing = cell.groove_timing.unwrap();
        if timing.values.iter().flatten().any(|v| {
            matches!(
                v,
                crate::temporal_cognition::feature_projection::Feature::Projected(_)
            )
        }) {
            positive_cells += 1;
        }
        new_intervals += timing.projected_records.iter().sum::<usize>();
        changed_coordinates += timing
            .values
            .iter()
            .flatten()
            .zip(timing_issue.timing_features.iter().flatten())
            .filter(|(a, b)| {
                a.value()
                    .zip(**b)
    ...
    assert!(
        positive_cells > 0 && new_intervals > 0 && changed_coordinates > 0,
        "cells={positive_cells} new_intervals={new_intervals} changed_coordinates={changed_coordinates}"
    );
    assert!(
        projected_ratings > 0 && changed_ratings > 0 && consumer_ratings > 0,
        "projected={projected_ratings} changed={changed_ratings} consumer={consumer_ratings}"
    );
    assert!(disabled.groove_issue(issue).is_none());
    assert!(enabled.groove_issue(issue + 1).is_none());
    println!(
        "candidate groove heads: projected_ratings={projected_ratings} changed_ratings={changed_ratings} consumer_cells={consumer_ratings}"
    );
    assert_eq!(
        observed_before,
        serde_json::to_value(enabled.diagnostics()).unwrap()
    );
    assert_eq!(
        phrase_before,
        serde_json::to_value(phrase.snapshot()).unwrap()
    );
    println!(
        "ordinary acoustic to candidate timing: positive_cells={positive_cells} new_intervals={new_intervals} changed_coordinates={changed_coordinates} fixed_test_association=true"
    );
    // A held, unassigned group's retirement must invalidate the entire frozen mixture.
    let frame = enabled.frame.as_mut().as_mut().unwrap();
    let empty = frame
        .evidence_groups
        .iter()
        .position(Option::is_none)
        .unwrap();
    let saved_owner = frame.next_owners[empty];
    let mut held = *frame
        .evidence_groups
        .iter()
        .flatten()
        .find(|g| g.ledger.group == group)
        .unwrap();
    held.ledger.group.generation = u64::MAX - 1;
    frame.evidence_groups[empty] = Some(held);
    frame.next_owners[empty] = Some(held.ledger.group);
    let mut retirement = action_profiles::Table::new();
    let with_held = retirement
        .refresh(&profiles, &phrase, &shared, &gesture, Some(&enabled))
        .unwrap();
    assert_eq!(
        with_held
    ...
    let frame = enabled.frame.as_mut().as_mut().unwrap();
    frame.evidence_groups[empty] = None;
    frame.next_owners[empty] = saved_owner;
    assert!(
        retirement
            .refresh(&profiles, &phrase, &shared, &gesture, Some(&enabled))
            .is_none()
    );
    assert!(retirement.publication(&profiles).is_none());
    assert_eq!(retirement.resources().invalidations, 1);
    assert!(
        table
    ...
    let before = serde_json::to_value(enabled.diagnostics().unwrap().groove_heads).unwrap();
    enabled.finish(720 * 512 + 1000).unwrap();
    assert!(enabled.groove_issue(issue).is_none());
    assert!(enabled.groove_issue(issue + 1000).is_none());
    assert_eq!(
        before,
        serde_json::to_value(enabled.diagnostics().unwrap().groove_heads).unwrap()
    );
    println!(
        "ordinary configured groove heads: supported_frames={supported} distinct_ratings={different} evidence_equal_frames=720 density_windows={density_windows} timing_coordinates={timing_coordinates} context_coordinates={context_coordinates} context_positive_cells={context_positive_cells}"
    );
}

pub(in crate::temporal_cognition) fn advance_synthetic_arrivals(
    model: &mut Option<Recurrence>,
    acoustic: &super::super::Snapshot,
    forecast: crate::config::TemporalPeriodConfig,
) {
    let first = acoustic.retained_groups[0].unwrap();
    let model = model.get_or_insert_with(|| {
        let mut cfg = config(first.bus);
        cfg.group.epoch = first.epoch;
        let mut controls = settings(128);
        controls.forecast = Some(forecast);
        Recurrence::new(space(), cfg, controls).unwrap()
    });
    let end = acoustic.assignment.end_sample;
    let mut evidence_groups = [None; 7];
    for (i, group) in acoustic.retained_groups.iter().copied().enumerate() {
        let Some(group) = group else { continue };
        let slot = &mut model.slots[i];
        if slot.owner.is_none() {
            // Synthetic ownership bypasses the acoustic admission path only in this fixture.
            slot.owner = Some(group);
            slot.estimator.reset(group, 0).unwrap();
            slot.grouping.reset(group, 0).unwrap();
        }
        assert_eq!(slot.owner, Some(group));
        let raw = &acoustic.features[i].as_ref().unwrap().raw;
        let accent = acoustic.features[i]
            .unwrap()
            .detector
            .and_then(|d| d.accent);
        if let Some(accent) = accent {
            slot.estimator.deliver(accent, end).unwrap();
        } else {
            slot.estimator.advance(end).unwrap();
        }
        slot.grouping.refresh(&slot.estimator).unwrap();
        let period = slot.estimator.view();
        let peak = period.peaks.iter().flatten().copied().max_by(|a, b| {
            a.support
                .total_cmp(&b.support)
        ...
        let context = arrival::Context {
            ...
            source_start: if support[2] == 0 {
                raw.source_start
            } else {
                support[0].min(raw.source_start)
            },
            source_end: support[1].max(raw.source_end),
            available: support[2].max(raw.available_end),
        };
        let forecast = slot
            .arrival
            .as_mut()
            .unwrap()
            .advance(Some(raw), accent, context, end, 48000)
            .unwrap();
        evidence_groups[i] = Some(GroupFrame {
            groove: None,
    ...
    model.residual.advance(end).unwrap();
    model.received_at = end;
    *model.frame = Some(Frame {
        end_sample: end,
        received_at: end,
        evidence_groups,
        residual: model.residual.summary(),
        next_owners: acoustic.retained_groups,
        newborn: [None; 7],
    });
}
