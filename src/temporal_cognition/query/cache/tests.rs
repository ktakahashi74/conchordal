...
#[test]
fn commitment_uses_original_owner_start_fresh_audio_and_pre_deadline_receipt() {
    let mut cache = Cache::new(1, 4, 8, 256);
    let mut scratch = Snapshot::default();
    let supported = entry(10, Some(0.2));
    let bindings = [Binding {
        identity: supported.identity,
        slot: 7,
        handle: 1010,
    }];
    for (id, start, end, audio, received, owner) in [
        (1, 0., 1., 1., 1.05, 9),
        (2, 0., 1.05, 1.05, 1.60, 9),
        (3, 0., 1.10, 1.10, 1.601, 9),
        (4, 0., 1.20, 1.20, 1.50, 9),
        (5, 0.1, 1.10, 1.10, 1.50, 9),
        (6, 0., 1.10, 0.99, 1.50, 9),
        (7, 0., 1.10, 1.10, 1.50, 99),
    ] {
        let mut q = header(id, end);
        q.start = start;
        q.audio_end = audio;
        q.occurrence_id = owner;
        scratch
            .pack(&q, &[entry(10, Some(0.2))], &bindings, received)
            .unwrap();
        cache.insert(&scratch).unwrap();
    }
    let selected = cache
        .for_commitment(9, [0, 1100], 1600, 1000, 2000)
        .unwrap();
    assert_eq!(selected.evidence().query_id, 2);
    assert_eq!(selected.evidence().received_at, 1.6);
    assert_eq!(selected.entry(7, 1010).unwrap().cost, Some(0.2));
    assert!(selected.entry(7, 1011).is_none());
    assert!(selected.entry(256, 1010).is_none());
    assert_eq!(
        cache
            .for_commitment(9, [0, 1100], 1599, 1000, 2000)
            .unwrap()
            .evidence()
            .query_id,
        1
    ...
    assert!(
        cache
            .for_commitment(9, [0, 1200], 1700, 1000, 2000)
            .is_some()
    );
    assert!(
        cache
            .for_commitment(9, [0, 1101], 1599, 1000, 2000)
            .is_none()
    );
    assert!(
        cache
            .for_commitment(9, [0, 1100], 1600, 1000, 1049)
            .is_none()
    );
    cache.clear(2, 5);
    assert!(
