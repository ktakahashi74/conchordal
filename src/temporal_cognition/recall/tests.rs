...
#[test]
fn actual_received_correspondence_candidates_keep_query_support_and_frozen_transforms() {
    use crate::temporal_cognition::joint::correspondence::Cache;
    let mut recall = Recall::new(1, 0, 1000, 100, config(), None).unwrap();
    let model = crate::temporal_cognition::section::tests::runtime_config();
    let group = input(1, 0.).retained_groups[0].unwrap();
    let mut cache = Cache::new(group, 1000, model, Some(0.)).unwrap();
    let mut parent = None;
    let mut compared = 0;
    let mut known_none = 0;
    ...
    for step in 1..=72 {
        ...
        let original = serde_json::to_vec(&recall.snapshot()).unwrap();
        cache
            .refresh(step * 100, recall.matches_for(group), &retained)
            .unwrap();
        let output = cache.proposals(parent, true, 0.1, &retained).unwrap();
        assert!(!output.parent_retired);
        for (i, state) in output
            .states
            .iter()
            .enumerate()
            .filter_map(|(i, s)| s.map(|s| (i, s)))
        {
            if !output.feature_supported[i] {
                continue;
            }
            let (q, rows) = recall.matches_for(group).unwrap();
            assert_eq!(state.support.query, q.query_id);
            assert_eq!(state.support.source_start, q.source_start_sample);
            assert_eq!(state.support.reconstruction_error, q.reconstruction_error);
            assert_eq!(state.support.search_candidates, q.candidates);
            assert_eq!(
                (
                    state.support.start,
                    state.support.end,
                    state.support.audio_end
                ),
                (
                    q.support_start_sample,
                    q.support_end_sample,
                    q.supporting_audio_end.unwrap()
                )
            );
            assert_eq!(
                (
                    state.support.available,
                    state.support.issued,
                    state.support.completed,
                    state.support.received
                ),
                (q.available_at, q.issued_at, q.completed_at, q.received_at)
            );
            assert_eq!(
                (state.support.cutoff_tie, state.support.search_pruned),
                (q.cutoff_tie, q.pruned_candidates)
            );
            if let Some(m) = state.matched {
                let original = rows
                    ...
                    .flatten()
                    .find(|old| {
                        (old.episode_id, old.episode_generation, old.anchor)
                            == (m.episode_id, m.episode_generation, m.anchor)
                            && old.transformation == m.transformation
                    })
                    .unwrap();
                assert_eq!(
                ...
                assert_eq!(state.log_score, original.acoustic_score(&model).unwrap());
                assert!(retained.contains(&(m.episode_id, m.episode_generation)));
                assert!(
                    m.support_end_sample <= q.support_start_sample && m.available_at <= q.issued_at
                );
                compared += 1;
                query_ids.insert(q.query_id);
            } else {
                known_none += 1;
                assert_eq!(state.id, 0);
            }
        }
        let sum: f64 = output.list.entries[..output.list.len]
            .iter()
            .flatten()
            .map(|c| c.log_weight.exp())
            .sum();
        assert!((sum - 1.).abs() < 1e-12);
        let selected = output.list.entries[..output.list.len]
            .iter()
            .flatten()
            .find_map(|c| c.id.filter(|&id| id != 0));
        parent =
            selected.and_then(|id| output.states.iter().flatten().find(|s| s.id == id).copied());
        assert_eq!(serde_json::to_vec(&recall.snapshot()).unwrap(), original);
    }
    assert!(
        compared > 20 && query_ids.len() > 5 && known_none > 0,
        "matches={compared} queries={} none={known_none}",
        query_ids.len()
    );
    recall.finish(100_000).unwrap();
    let retained = recall.retained_ids();
    cache
        .refresh(100_000, recall.matches_for(group), &retained)
        .unwrap();
    let unknown = cache.proposals(None, true, 0.1, &retained).unwrap();
    assert_eq!(unknown.list.len, 1);
    eprintln!(
        "JOINT_CORRESPONDENCE_REAL matched_candidates={compared} queries={} known_no_memory={known_none}",
        query_ids.len()
    );
}

#[test]
fn actual_correspondence_search_coverage_distinguishes_empty_memory_capacity_and_bounds() {
    use crate::temporal_cognition::joint::correspondence::Cache;
    let model = crate::temporal_cognition::section::tests::runtime_config();
    let group = input(1, 0.).retained_groups[0].unwrap();
    let mut empty = 0;
    ...
    for shifted in [false, true] {
        let mut cfg = config();
        cfg.candidates = Some(if shifted { 16 } else { 1 });
        let mut recall = Recall::new(1, 0, 1000, 100, cfg, None).unwrap();
        let mut cache = Cache::new(group, 1000, model, Some(0.)).unwrap();
        for step in 1..=80 {
            let mut frame = input(step, 0.);
            if shifted && step > 16 {
                frame.features[0].as_mut().unwrap().raw.values[0] = Some(12.);
            }
            recall.advance(&frame, step * 100, None).unwrap();
            let retained = recall.retained_ids();
            let Some((q, rows)) = recall.matches_for(group) else {
                continue;
            };
            cache
                .refresh(step * 100, Some((q, rows)), &retained)
                .unwrap();
            let options = cache.proposals(None, true, 0.1, &retained).unwrap();
            let no_memory = options.states.iter().flatten().any(|s| s.id == 0);
            if q.candidates == 0 {
                assert!(q.search_covered);
                assert!(no_memory);
                empty += 1;
            }
            if q.pruned_candidates > 0 {
                assert!(!q.search_covered);
                assert!(!no_memory);
                capacity += 1;
            }
            if shifted && q.candidates > 0 && q.best.is_none() {
                assert!(!q.search_covered);
                assert!(!no_memory);
                assert_eq!(options.list.len, 1);
                bounds += 1;
            }
        }
    }
    assert!(
        empty > 0 && capacity > 0 && bounds > 0,
        "empty={empty} capacity={capacity} bounds={bounds}"
    );
    eprintln!("JOINT_CORRESPONDENCE_COVERAGE empty={empty} capacity={capacity} bounds={bounds}");
}
