...
#[test]
fn joint_section_law_uses_owned_duration_context_and_covariates() {
    use crate::temporal_cognition::{
        joint::proposals::{Boundary, List},
        section::{Head, commitment::ContextSupport},
    };
    let parent = filled();
    let mut head = Head {
        means: [0.; 82],
    ...
    head.hazard[0] = 0.7_f64.exp_m1().ln();
    head.exits[0][0] = 2_f64.ln();
    head.exits[2][0] = -2_f64.ln();
    head.validate().unwrap();
    let mut cases = 0;
    let mut maximum_error = 0_f64;
    for dt_ms in [10, 100, 1000, 2000] {
        for returned in [
            None,
            Some(ContextSupport {
                context_id: 40,
            ...
            let dt = dt_ms as f64 / 1000.;
            let list = List::owned_active_section(
                [10, 11, 12, 13],
                &parent,
                [4000, 4000 + dt_ms],
                1000,
            ...
            .unwrap();
            let survival = (-0.7 * dt).exp();
            let exit = -(-0.7 * dt).exp_m1();
            let raw = [
                survival,
                exit * 2. / 3.5,
                exit / 3.5 * returned.map_or(0., |r| r.mass),
                exit * 0.5 / 3.5,
            ];
            let total: f64 = raw.iter().sum();
            for choice in list.entries.iter().flatten() {
                let expected = if let Some(id) = choice.id {
                    raw[(id - 10) as usize] / total * (-dt / 120.).exp()
                } else {
                    -(-dt / 120.).exp_m1()
                };
                maximum_error = maximum_error.max((choice.log_weight.exp() - expected).abs());
                if choice.id == Some(12) {
                    assert_eq!(
                        choice.boundary,
                        Some(if returned.unwrap().context_id == 40 {
                            Boundary::Stay
                        } else {
                            Boundary::Exit
                        })
    ...
    assert!(
        List::owned_active_section([10, 11, 12, 13], &parent, [3900, 4000], 1000, None, &head)
            .is_err()
    );
    println!("JOINT_SECTION_LAW cases={cases} maximum_error={maximum_error:e}");
}
