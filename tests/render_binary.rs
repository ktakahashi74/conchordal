...
fn conchordal_render_skips_zero_amp_note_on_and_still_writes_audio() {
    let scenario = write_inline_scenario(
        r#"
let silent = place(sine().pitch_mode("lock"), at(220.0)).amp(0.0);
let audible = place(sine().pitch_mode("lock"), at(330.0)).amp(0.25);
flush();
wait(0.4);
release(silent);
release(audible);
...
fn conchordal_render_fails_when_wav_thread_panics() {
    let scenario = write_inline_scenario(
        r#"
let tone = place(sine().pitch_mode("lock"), at(220.0)).amp(0.2);
flush();
wait(0.2);
release(tone);
wait(0.1);
