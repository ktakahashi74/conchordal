...
fn prediction_decay_reduces_far_extrapolation() {
    ...
    world.observe_consonance_field_level01(prev_tick, Arc::from(prev_scan));
    world.observe_consonance_field_level01(last_tick, Arc::from(last_scan));

    let near_tick = last_tick.saturating_add(tau_tick);
    let far_tick = last_tick.saturating_add(tau_tick.saturating_mul(2));
    let pred_near = world
        .predict_consonance_field_level01_at(near_tick)
        .expect("pred near");
    let pred_far = world
        .predict_consonance_field_level01_at(far_tick)
        .expect("pred far");
