...
fn world_model_pred_cache_resets_on_updates() {
    ...
    world.observe_consonance_field_level01(time.frame_end_tick(0), scan);
    assert!(world.predict_consonance_field_level01_next_gate().is_some());
    ...
    world.observe_consonance_field_level01(time.frame_end_tick(1), scan2);
    let _ = world.predict_consonance_field_level01_next_gate();
    assert!(world.last_pred_next_gate().is_some());

    let scan3 = Arc::from(vec![0.3f32; space.n_bins()]);
    world.observe_consonance_field_level01(time.frame_end_tick(2), scan3);
    assert!(world.last_pred_next_gate().is_none());
}
