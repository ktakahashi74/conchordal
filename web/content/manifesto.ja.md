...
Conchordalの基盤となる「ランドスケープ」は、人間の聴覚認知特性を模したポテンシャル場である。従来の作曲が格子状の音階や固定された音符に音を当てはめるのに対し、Conchordalは人間の聴覚生理そのものを仮想環境として再現する。現在は実音から周波数方向の地形を計算し、短時間の音響履歴と拍の推定を時間的な振る舞いに用いている。時間方向にも、対象とする聴覚認知機構の状態と動態を地形として表し、聞こえた音同士の時間的な関係を、各エージェントの発音判断へ返すことを目指す。
...
音楽の時間構造には、一音の立ち上がりや余韻、拍の中で音が応答する位置、フレーズの継続と終止、長い隔たりを越えた反復や変奏がある。Conchordalは、これらを支える神経認知的な時間構造とその相互作用を内部で表現することを設計目標とする。聞こえた関係を保持して次の音を期待し、その期待が個体の局所的な発音判断へ戻ることで、異なる時間尺度をつなぐ構想である。

2026年9月時点の実装では、拍と細分拍の推定からデルタ・シータと名づけた周期信号を作り、アルファ・ベータは推定の確かさと予測誤差の指標を表す数値として扱っている。音響信号や脳信号から四つの神経帯域を抽出しているわけではない。個体は自身の発声周期と音長を持ち、聞こえた音の短い文脈を使って発音・待機・見送りを選ぶ。明示的な同期が指定された場合には拍への同調を使う。

以下の四帯域への機能配分は初期の案であり、再検討の対象である。帯域の名称や周波数範囲は、実装された抽出器や認知機能を示すものではない。

* **デルタ帯域 (~0.5–4 Hz)**
  緩やかなパルスと拍節構造、大局的なフレージング（楽句の呼吸）を対応づける案。

* **シータ帯域 (~4–8 Hz)**
  音のアーティキュレーションや音節単位のまとまり、個々の音事象の輪郭を対応づける案。

* **アルファ帯域 (~8–13 Hz)**
  フレーズ内部の拍動やアクセントのパターンを対応づける案。

* **ベータ帯域 (~13–30 Hz)**
  グルーヴ感や複数音の細かな時間関係（マイクロタイミング）を対応づける案。

長い隔たりを越えた再認や期待を扱う場合には、その応答を支える記憶や想起の仕組みを選ぶ。何を保持し、どのように更新し、生成へ作用させるかを神経認知的な根拠と照合し、対象とする応答に必要な表現を定める。楽曲全体を記憶することを、あらゆる生成の前提にはしない。拍への同時発音、異なる位置での応答、拍を参照しない流れを区別し、その違いが音楽として聞こえるところまでを目標に据える。
...
* **個体 (Individual)**
  ランドスケープ上で活動する基本単位。各個体は自律的な音響シンセサイザーであり、周囲のポテンシャル勾配を知覚し、自身の発声状態と短い音響文脈に応じて発音の機会を選び、エネルギーを消費しながら生存する。個体は協和度の高い領域を求めて周波数空間を移動し、不協和な領域では急速に消耗する。中央集権的な指揮者は存在しない。全ては局所的な知覚と反応によって決定される。

* **ポピュレーション (Population)**
  複数の個体が形成する音響の群集。個体は互いに干渉し、周波数空間で競合を避けてニッチを分化させるか、倍音関係にある領域で共生的に融合する。群集の密度はランドスケープ自体を動的に変形させ、環境と住人は相互に規定し合う。時間方向にも異なる役割を保ち、受け渡しや反復・変奏を生み出す群集を目指す。ポピュレーションは単なる個体の総和ではなく、集団としての創発的性質を持つ。

個体の局所的行動から、和声、旋律、リズムといった音楽的構造をボトムアップに生み出すことが、この生態系の目標である。現在の音高の適応と短時間の発音判断を足場に、持続する受け渡しやフレーズの関係を扱う機構を検討していく。作曲者が音符を配置するのではなく、音響生態系の力学が音楽を生み出すのである。

### シナリオ： 時間に形を与える

始まりと終わりを持つ作品では、創作者が時間を有限な持続として切り取り、緊張と解放、上昇と下降、クライマックスと静寂といった構造を与える。Conchordalでは、局所的な調和やリズムの創発を通じて作品をひとつの体験として成立させる大きな弧を、人間の創作者がデザインする。作曲者は音符を一つ一つ配置するマイクロマネージャーではなく、生態系の演出家として振る舞う。

この演出のための道具が時間軸に沿って生態系の環境条件を変化させる指示書である「シナリオ」である。創作者は個々の音を指定するのではなく、系全体の状態遷移を記述する。ランドスケープの地形を変形させ、個体群の生成と消滅を制御し、発声の周期、音長、同期の設定を時間に沿って変える。これらの操作を通じて、創作者は混沌から秩序へ、あるいは静寂から飽和へといった巨視的な軌道を描くことができる。知覚されたフレーズや過去の音が局所生成へ及ぼす作用は、対象とする認知機構に基づいて検討する。作者が記述した構成と、それを聴取者がどう経験するかは区別して扱う。

始まりと終わりを持つ作品に加え、終端を定めずに音響生態系を動かし続ける利用形態も構想する。そこでは、各個体がその時点までに受け取った音と現在の環境条件に応じて活動を続け、局所的なまとまりや変化を生む。生成を支える認知状態は扱う応答に応じて定め、予定された曲長や全曲の完了感を必要条件にしない。
