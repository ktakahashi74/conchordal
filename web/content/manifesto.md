...
When recognition or expectation across long interruptions is a target, its memory and retrieval mechanisms must be selected. What is retained, how it is updated, and how it affects generation will be examined against neurocognitive evidence to define the representation that response requires. Remembering the whole piece is not a prerequisite for every form of generation. The goal includes audible distinctions between simultaneous attacks on a beat, responses at different positions, and flow without a beat reference.
...
The ecosystem's goal is to produce musical structures, including harmony, melody and rhythm, from the local behaviors of Individuals. Current pitch adaptation and short-term phonation decisions provide a starting point for investigating mechanisms of sustained handoffs and phrase relations. It is not the composer placing notes but the dynamics of an acoustic ecosystem that generate music.

### Scenario: Giving Shape to Time

In a work with a beginning and an end, the creator shapes a finite duration through tension and release, rise and fall, climax and silence. In Conchordal, a human creator designs the large arc through which locally emerging harmony and rhythm form a coherent experience. The composer acts not as a micromanager placing each note but as a director of the ecosystem.

The tool for this direction is the *Scenario*: a set of instructions that modulates the ecosystem's environmental conditions along the time axis. The creator specifies not individual sounds but state transitions of the entire system: deforming the landscape's topography, controlling the generation and extinction of populations, and changing phonation periods, sound durations and synchronization settings over time. Through these operations, the creator can trace macroscopic trajectories from chaos to order, or from silence to saturation. The influence of perceived phrases and earlier sounds on local generation will be investigated through the selected cognitive mechanisms. Authored structure and the way a listener experiences it are treated separately.

Alongside finite works, Conchordal envisages an acoustic ecosystem running without a predetermined end. In that use, each Individual continues acting in response to the sounds it has received up to the present and the current environmental conditions, producing local groupings and changes. The cognitive state supporting generation is chosen for the response being modeled; planned duration and whole-piece completion are not prerequisites.
