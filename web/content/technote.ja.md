...
それでも実証的に生き残ったものがある。`mirror_weight` を上げると、倍音を放射する身体にとって地形の整合性が崩れ、これは*調波的な緊張*として聴こえる。フラッグシップ・エチュードはまさにこの性質を使い、`harmonic_tension` のアーチを楽曲の緊張と解決の軸に据えている（7.3節）。調性のスイッチとしてではない。
...
*   調波地形: `harmonic_tension(v)`（倍音/下倍音重力のブレンド）, `set_roughness_k(v)`, `set_pitch_objective("consonance"|"dissonance")`。
...
エチュード集を締めくくるこの一曲は、スタック全体をひとつの有向アークとして構成する。作曲家が触れるマクロなノブは二つだけである。`harmonic_tension` のアーチ（協和 → 不協和 → 協和）と、レジスタの移調。残りはすべて創発に委ねられる——深いビートアトラクタを駆動するメトリックな鼓動（アクセントロールのボイス）、協和へ登りながら*同じ*創発ビートにロックする生きたコロニー、緊張の頂点でだけ現れる非拍節のフローのきらめき。そして解決とは、コロニーが不協和の頂点を生き延びること（協和性でゲートされた生存可能性と、協和性に偏ったリスポーン）にほかならない。調的重力の回帰はスコアに書かれているのではなく、エコシステムによって演じられる。
