...
2.  **状態A（長調）**: `harmonic_mirror(0.0)`。共通基音投影（倍音列）を使用する。協和性を追求するエージェントは E4 と G4 の周辺に集まり、C長三和音を形成する。
3.  **状態B（短調）**: `harmonic_mirror(1.0)`。共通倍音投影（下倍音列）に切り替わる。地形の「重力」が反転する。エージェントは Ab3 と F3 で安定性を見出し、フリジア旋法/短調のテクスチャを生成する。Conchordal における「調性」が温度や重力に類似した操作可能な環境変数であることを示す。
...
## 7.4 ケーススタディ：Emergence and Resolution（`samples/04_ecosystems/conchordal_flagship.rhai`）
