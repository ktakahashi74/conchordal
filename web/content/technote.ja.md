...
*   **パスA（倍音）**: 上で述べた標準の「下方→上方」投影。倍音列に基づく重力を生み、仮想ピッチを周波数領域で実現したものにあたる。
*   **パスB（下倍音）**: これを反転した「上方→下方」投影。共通の倍音を見つけ、そこから下倍音を投影する。短調を長調の鏡像とみなす Riemann の*和声二元論*の実装である。

$$ H_{final} = (1-\alpha)H_{overtone} + \alpha H_{undertone} $$

ただしパスBの解釈には、三つの点で注意が要る。第一に物理の問題。下倍音列は自然界に存在せず（振動体が放射するのは倍音である）、下倍音地形には刺激側の対応物がない。第二に知覚の問題。「共通倍音による束縛」に相当する知覚機構は、仮想ピッチに比肩するものとしては知られていない。現代の心理音響学は短三和音を「仮想基音が弱く曖昧な和音」と説明するのであって、長三和音の鏡像とは説明しない。第三に、最も決定的な生態学の問題。この系は閉ループであり、二元論は生成側で破れる。倍音位置に引き寄せられたエージェントは、自分を引き寄せた基音構造をその放射スペクトルでさらに強める（アトラクタが自己安定化する）。ところが下倍音位置に引き寄せられたエージェントが放射するのもやはり倍音列であって、自分を引き寄せた共通倍音構造を強めはしない。つまり帰還ループが閉じない。実際、論文の統制実験では倍音側のクラスタリングは再現されたが、下倍音（短調）側の再組織化は再現されず、当初の「長調/短調スイッチ」という仮説は放棄された。

それでも実証的に生き残ったものがある。`mirror_weight` を上げると、倍音を放射する身体にとって地形の整合性が崩れ、これは*調波的な緊張*として聴こえる。フラッグシップ・エチュードはまさにこの性質を使い、`harmonic_mirror` のアーチを楽曲の緊張と解決の軸に据えている（7.3節）。調性のスイッチとしてではない。
...
**クラウディング**: クラウディング（相互排斥）とピッチ適応は、単一の共有された占有場（occupancy field）を参照する。各占有周波数を中心とするガウシアンペナルティで、その幅は `crowding_sigma_cents`（デフォルト 60）が定め、強度は `crowding_strength` が制御する。さらに作曲家ノブ `octave_avoidance` で重み付けしたオクターブ等価（クロマ）項を加えると、周波数差を1オクターブに畳み込み、オクターブやユニゾンの重複を排斥しつつ異なる和音構成音は自由に残す：`occupancy = Gaussian(Δf₀) + octave_avoidance · Gaussian(Δ_chroma)`。これにより、エージェント同士が同一周波数に収束して「団子」状態になるのを防ぎ、スペクトル上の多様性を促す。
...
*   クラウディング: `avoid_neighbors(strength)`（デフォルトのシグマ）／ `avoid_neighbors(strength, sigma_cents)`, `octave_avoidance(weight)`（オクターブ・ユニゾン重複の排斥）, `crowding_target(same, other)`, `leave_self_out(bool)`, `leave_self_out_mode("approx"|"exact")`。
...
エチュード集を締めくくるこの一曲は、スタック全体をひとつの有向アークとして構成する。作曲家が触れるマクロなノブは二つだけである。`harmonic_mirror` のアーチ（協和 → 不協和 → 協和）と、レジスタの移調。残りはすべて創発に委ねられる——深いビートアトラクタを駆動するメトリックな鼓動（アクセントロールのボイス）、協和へ登りながら*同じ*創発ビートにロックする生きたコロニー、緊張の頂点でだけ現れる非拍節のフローのきらめき。そして解決とは、コロニーが不協和の頂点を生き延びること（協和性でゲートされた生存可能性と、協和性に偏ったリスポーン）にほかならない。調的重力の回帰はスコアに書かれているのではなく、エコシステムによって演じられる。
...
*   **鏡像双対性から、生成ループの不動点要件へ。** 下倍音地形は計算できるのに、短調の調性は創発しなかった。この分析は一般化できる。知覚的な対称性が音楽として実在するのは、それに引き寄せられたエージェントの放射スペクトルが、その対称性を強め返す場合に限られる。知覚は鏡像化できても、生成はできない。あらゆる身体は倍音を放射するからである（3.3.3節）。したがって今後のあらゆる地形操作は、二つの試験に合格しなければならない——知覚機構が実在すること、そして生成側でループが閉じること。
...
| `octave_avoidance` | 占有場 | Float ≥ 0 | オクターブ・ユニゾン重複を排斥するクロマ項の重み。0 でオクターブは収束自由。 |
