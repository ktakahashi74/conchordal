...
両メトリクスは$[0, 1]$範囲に正規化される。協和性はその後2つのレイヤで導出される：観測量を単一の適応度スコアに融合する**協和性カーネル**と、そのスコアを下流の各コンシューマ向けに変換する**表現変換**の組み合わせである。

**レイヤ1 — 協和性カーネル(双線形族):**

$$ C_{score} = a \cdot H_{01} + b \cdot R_{01} + c \cdot H_{01} R_{01} + d $$

デフォルト係数：$a = 1.0$、$b = -1.35$、$c = 1.0$、$d = 0.0$。$b < 0$であるため粗さはペナルティとして作用し、$c > 0$であるため高い調波性がそのペナルティを緩和する（相互作用項$c \cdot H_{01} R_{01}$は$H_{01}$が大きいとき$b \cdot R_{01}$を部分的に相殺する）。双線形族は、$c = 0$の特殊ケースとして以前の$\alpha H - wR$定式化を包含する。
...
| $C_{weight}$ | $\exp(C_{score}/T) + \varepsilon$ | $[0,+\infty)$ | サンプリング用ボルツマン重み |
| $C_{density}$ | $\text{normalize}(C_{weight})$ | $[0,1],\;\Sigma=1$ | ピッチ選択PMF |
| $C_{energy}$ | $-C_{score}$ | $(-\infty,+\infty)$ | 最小化用エネルギー |

ここで$\sigma(x) = 1/(1+e^{-x})$、$\beta$はシグモイドの急峻さを制御し（デフォルト2.0）、$\theta$はシグモイドの閾値である（デフォルト0.0）。
...
* **持続**:エージェントは`metabolism_rate`(1秒あたりのエネルギー損失)を持ち、協和性依存の充電を介してエネルギーを得ることができる。
    * **充電**:これが重要なフィードバックループである。phonation攻撃ごとに得られるエネルギーは、`MetabolismPolicy`を介してエージェントの$C_{level01}$（シグモイド変換された協和性）によってスケールされる。
    * **生存**:不協和な(低$C_{level01}$)領域のエージェントは「飢える」—そのエネルギーが枯渇し、振幅が減少し、最終的に死亡する。協和な(高$C_{level01}$)領域のエージェントは「食べる」—エネルギーを維持または獲得し、より大きく長く歌うことを可能にする。
...
| `kernel.a` | `ConsonanceKernel` | Float | 調波性係数（デフォルト1.0）。 |
| `kernel.b` | `ConsonanceKernel` | Float | 粗さ係数（デフォルト−1.35；負値は粗さにペナルティを課す）。 |
| `kernel.c` | `ConsonanceKernel` | Float | 相互作用係数（デフォルト1.0；正値は高調波性で粗さペナルティを緩和）。 |
| `kernel.d` | `ConsonanceKernel` | Float | バイアス項（デフォルト0.0）。 |
| `beta` | `ConsonanceRepresentationParams` | Float | $C_{level01}$のシグモイド急峻さ（デフォルト2.0）。 |
| `theta` | `ConsonanceRepresentationParams` | Float | $C_{level01}$のシグモイド閾値（デフォルト0.0）。 |
