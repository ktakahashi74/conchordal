...
## 7.1 ケーススタディ：リズム族連続体（`samples/04_ecosystems/metric_beat_foundation.rhai`, `entrained_beat.rhai`, `flow_timing_field.rhai`）

3つの姉妹スクリプトが、1つの結合機構（5.4節）が質的に異なる時間性を張ることを示す。

1.  **Metric**（`metric_beat_foundation.rhai`）: ボイスは `metric()` を宣言し、ダウンビート担当のボイスにアクセントロールを与える。それらのオンセットが共有の生成メーターを駆動し、メーターの確信度が上昇し、高い結合が全オンセットを深くなったアトラクタへ引き込む。読み取れるパルスが現れる——しかしどこにもクロックは存在せず、あるのは地形にテンポの落ち着き先を告げる `temporal_basin` だけである。
2.  **Entrained**（`entrained_beat.rhai`）: 中程度の結合に活力結合とアタック報酬を加えたもの。同期は即座ではなく、確信度の蓄積につれて数十秒かけて*創発*し、コロニーが弱れば劣化する——リズム的凝集が生態学的健康と結びついている。
3.  **Flow**（`flow_timing_field.rhai`）: ほぼゼロの結合と高い `flow_depth`。オンセットはクラスタ化された更新過程に従う——屋根を打つ雨——構成からして非拍節であり、その間もピッチ挙動は協和性フィールドに乗り続ける。

## 7.2 ケーススタディ：漂流と流れ（`samples/04_ecosystems/drift_flow.rhai`）
...
今後の開発は、メーターレベルのアクセント構造（小節の強調）、空間化（ランドスケープの3D拡張）、進化遺伝学（成功したエージェントによる `TimbreGenotype` の継承）、そして完全な Direct Cognitive Coupling 閉ループへ向けたバイオシグナルフィードバックに焦点を当てる。
