...
「ライフエンジン」はDSPランドスケープの上で実行されるエージェントベースのシミュレーション層である。「ボイス（Voice）」の集団を管理し、そのライフサイクル、感覚処理、およびアクチュエーション（オーディオ合成）を処理する。

## 4.1 ボイス（Voice）アーキテクチャ
...
*   `KuramotoCore`: メーター由来のリズム帯域（5.3節）に同期する「呼吸」振動子。エネルギー/活力モデルを内蔵し、`rhythm_coupling`（`TemporalOnly` / `TemporalTimesVitality`）、`rhythm_reward`（リズム同期による代謝報酬）、`autonomous_attack`（自律的アタックトリガ）を備える。実効結合強度 $k_{eff}$ は活力と結合モードから計算される。
...
Conchordal はこの説明を文字通り採用する。マスタークロックやメトロノームの概念を排除し、時間は**創発メーター**——神経共鳴モデルを実装した結合リミットサイクル振動子ネットワークが、エコシステム自身のオンセットを*聴き*、そこからメトリカルな知覚を形成する——によって構造化される。ボイスは今度はその知覚されたパルスに対し、ボイスごとの結合強度でオンセットタイミングを同期させる。すなわちリズムは閉じた知覚–行為ループである：何もビートをスケジュールしない。ビートは集団の振る舞いから凝縮し、そして集団を引き込む。これは調波ランドスケープの時間軸版の鏡像である：協和性が課されるのではなく蝸牛のモデルから計算されるのとまったく同様に、メーターは課されるのではなくビート知覚のモデルから計算される。

## 5.1 メーターコア：強制リミットサイクル振動子

`MeterNetwork`（`core/meter.rs`）はビート振動子を強制 Hopf 標準形として保持し、極座標 $(r, \varphi)$ で積分する：

$$ \dot{r} = \alpha r + \beta r^3 + F_a\, s(t) \cos\varphi $$
$$ \dot{\varphi} = \omega - F_p \frac{s(t)}{r} \sin\varphi $$

$\alpha > 0$、$\beta < 0$ のとき、非強制系は半径 $\sqrt{-\alpha/\beta} = 1$ の安定リミットサイクルを持つ：ビートは**自己持続的**であり、入力の途切れを惰性で乗り越える（ビート誘導の持続レジーム）。駆動 $s(t)$ は整流オンセット信号で、`DorsalStream`（`core/stream/dorsal.rs`、3帯域クロスオーバーのフラックス検出器）が抽出するスペクトルフラックスと、集団自身の発声オンセット強度を組み合わせたもの——低遅延の聴覚–運動強化経路である。

振動子の固有周波数は可塑的である。ヘッブ学習則が刺激への位相誤差を減らす方向に $\omega$ をシフトする：

$$ \dot{\omega} = -\eta\, s(t) \sin\varphi $$

ランダム（更新過程的）な入力は平均してシフトを打ち消すため、ビートはノイズを追いかけない。周期的な入力はビート帯域（0.5–4 Hz）内で $\omega$ を刺激レートへ引き込む。ビートの上にネットワークは同期した**サブディビジョン**と遅い**小節**サブハーモニック（アンラップされたビートカウントに対する比 2, 3, 4）を追跡し、3階層のメトリカル状態（`MeterState`）を与える。
...
メーターは単にビートを追跡するだけでなく、ビートが*どれだけ存在するか*を知っている。検出された各オンセットは現在のビート位相の単位フェーザをリーキー積算器に堆積させ、その合成ベクトル長が位相ロック値（PLV）となる。確信度は PLV を *presence* 項でゲートしたものであり、飽和にはおよそ4オンセット分の証拠を要する——ビート誘導に数サイクルを要するという心理学的観察に対応する——そして無音中は減衰する。散乱したオンセット位相は合成ベクトルを低く保つため、密度だけで確信度を捏造することはできない。

## 5.3 メーターから変調へ：NeuralRhythms
...
ここで $\kappa$ はボイスの結合強度（`entrainment`、0–1）、$c$ はメーターのビート確信度である。位相プルが振動子の交差をビート位相（`microtiming` でオフセット可能）へ引き寄せる：
...
ここで $s_\theta$ はボイスのシータ感度、$|\theta_{mag}|$ と $\theta_\alpha$ はビート確信度に由来し、$g_{env}$、$a_{env}$ はエンベロープゲートと振幅である。ヘルパー関数 `kuramoto_k_eff()` と `kuramoto_phase_step()` は外部シミュレーション（論文実験）用に公開されている。エネルギー/活力サブシステムはこの結合と相互作用する。
...
*   時間地形: `meter_stability(v)`, `temporal_basin(min_hz, max_hz)`（5.6節）。
