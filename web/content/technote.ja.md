...
*   **Error**: Voice は SoundBody の現在のピッチと計画目標との間の不一致（符号付きセント、絶対セント）を計算する。これは以前の行動の*結果*を表し、観測に利用できる。PitchCore はエラー信号を読み取らない——探索はフィードバックから切り離されたままである。
...
**クラウディング**: クラウディング（相互排斥）とピッチ適応は、単一の共有された占有場（occupancy field）を参照する。各占有*基音*（生 f0）を中心とするガウシアンペナルティで、その幅は `crowding_sigma_cents`（デフォルト 60）が定め、強度は `crowding_strength` が制御する。これにより、エージェント同士が同一周波数に収束して「団子」状態になるのを防ぐ。オクターブ等価（クロマ）のペナルティは適用せず、オクターブ関係は協和ポテンシャルを通してだけ作用する。
...
残りの波形を解析して周囲のエネルギー履歴を作る。従来の共有周期・係数による射影と、局所RMS履歴による予測を候補とし、発行後の実測で比較した混合を使う。

行動方策は、予定近傍で鳴る、参加の1周期まで待つ、今回は見送る、を比較する。
候補のコストは、参加周期を単位とした変位の二乗、経験した周囲のエネルギー形状との距離、
三つの帯域の重なり積分からなる。経験文脈は発音後の二つの実測窓から更新し、
候補の予測も同じ窓のエネルギーを対象にする。音長規則の保持時間とVoiceのADSR releaseを64点で
...
予測は未決定の行動に必要な場合だけ作り、経験した周囲の実音は次の候補比較へ戻す。
判断時までに観測した周囲のRMS履歴も、過去への遅れと観測範囲を伴う不変の記録として予測へ添える。
現在の候補コストは二窓の経験文脈だけを使う。
持続中の音の終了判断、行動の介入効果と価値の学習は未実装である。
...
2. **Entrained**（`08_murmuration.rhai`）: 音の文脈を使い、自身のペースから参加する。Gated発音の回復は協和度に基づき、位相一致報酬を使わない。
3. **Flow**（`09_rain.rhai`）: 不規則な身体の更新過程と、音の文脈への弱い応答を使う。音高は引き続きConsonance Fieldに従う。

08と09には同期を補助する専用の拍打ちはない。

## 7.2 ケーススタディ：漂流と流れ（`samples/research/drift_flow.rhai`）

この短い研究用fixtureは、Populationへのlive patchとfield-relative placementの反復を示す。自律的なdriftを示すものではない。
...
自律的なsettlingは`samples/05_settling.rhai`が担う。
...
「緊張」「解放」は構成の意図を示す名前であり、知覚上の達成を確認した判定ではない。和声展開と終止感は、生成条件に対応する試聴で別に評価する。
...
v0.4.0は論文の知見を楽器そのものに統合し、アーキテクチャに時間軸を加えた。固定のリズムフィルタバンクは、ヘッブ的なテンポ学習とPLV確信度を備えた強制リミットサイクル振動子——創発メーター——に置き換わった。Voiceのタイミングは、同期する`metric`と、音響への参加である`entrained`・`flow`の意図として表す（5.4節）。そして作曲家による時間の制御は、パルスをスケジュールすることなくその形成場所だけを形づくる地形事前分布（`meter_stability`、`temporal_basin`）へと還元された。デュアルバス設計は、ハビタット（エコシステムが感じるもの）とプレゼンテーション（聴衆が聴くもの）を分離する。このreleaseには、任意で有効化するLandscape habituationと、pitch-search temperatureを上げられる`ListenerTwin`の模擬feedback pathも含まれる。どちらもデフォルトでは挙動を変えない。
...
Manifesto は公約を宣言する。本章は、現在の実装がそのうち何を果たし、何が未解決かを記録する。ここに書くのは、作られた楽器の姿である。各行の背後にある設計判断、数値登録、改訂の履歴はリポジトリの設計台帳 [`docs/design-notes/technote-ledger.ja.md`](https://github.com/ktakahashi74/conchordal/blob/main/docs/design-notes/technote-ledger.ja.md) に置き、本章の旧節番号（§9.2–§9.4）をそこで保持する。
...
| 中央指揮者の不在 | ローカルな知覚のみ。メーターは集団自身のオンセットから創発する | §4–5, §7 | 実装済み。専用の拍打ちは明示的な同期の実演（sample 07）に限る |
| シナリオ=マクロの演出 | ディレクターの地形操作 | §6.3.6 | 作者による構成として実装済み |
| 時間構造を聴覚認知に根差させる | 順序付きの聴覚観測、アーティキュレーションの gesture、周期・到来予測、Voice の動作に条件づけた自声予測が、既定 off の診断としてエンジン内に存在する | — | 部分。いずれも生成には作用しておらず、本ノートの説明対象外。phrase、section、長い文脈の記憶は楽器本体の外の研究 |
...
- ランドスケープのパラメータとしての文化的音律体系と個人の聴覚特性。
- `entrained`・`flow` における、演奏条件をまたいだ時間関係の持続。
- 生成に作用する最初の時間認知機構。現在は一つも作用していない。
- phrase、section、長い文脈の記憶。楽器本体の外の研究として保留。
