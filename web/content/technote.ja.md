...
**GateSnap vs. Glide**: ピッチ移動は `PitchApplyMode` で制御される。`GateSnap` はノート境界でピッチを離散的にスナップし（各ノートは単一の安定周波数で鳴る）、`Glide` は連続的なポルタメントを使用する。
...
*   ピッチ: `pitch_mode("free"|"lock")`, `seek_consonance()`, `pitch_core("hill_climb"|"peak_sampler")`, `pitch_apply_mode("gate_snap"|"glide")`, `glide(v)`, `landscape_weight(v)`, `tessitura_gravity(v)`, `exploration(v)`, `persistence(v)` ほか。
